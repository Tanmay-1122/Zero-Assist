...
mod memory_browse;
mod models;
mod pdf_generation_ffi;
mod piper_phonemizer;
mod repl;
mod runtime;
mod session;
mod shared_folder;
mod skill_generator;
mod skill_generator_ffi;
mod skills;
mod streaming;
mod task_classifier_ffi;
mod termux_bridge_client;
mod sandbox_bridge_client;
mod device_control_dispatch;
mod tools_browse;
mod traces;
mod types;
mod url_helpers;
...
/// Hot-swaps the running daemon's config from a fresh TOML string.
///
/// Parses the TOML and replaces the in-memory config without restarting
/// any supervisors. The new config takes effect on the next session start.
/// This allows MCP server changes and other config updates to be picked up
...
/// [`FfiError::InternalPanic`] if native code panics.
#[uniffi::export]
pub fn reload_daemon_config(config_toml: String) -> Result<(), FfiError> {
    catch_unwind(|| runtime::reload_config_inner(config_toml)).unwrap_or_else(|e| {
        Err(FfiError::InternalPanic {
            detail: panic_detail(&e),
        })
    })
...
/// One-shot LLM completion for internal planners (device-control, UI agent).
///
...
///
/// Returns [`FfiError::StateError`] if the daemon is not running,
/// [`FfiError::SpawnError`] on provider construction or chat failure,
/// or [`FfiError::InternalPanic`] on native unwind.
#[uniffi::export]
pub fn send_planner_completion(prompt: String) -> Result<String, FfiError> {
    catch_unwind(|| {
        if estop::is_engaged() {
            return Err(FfiError::EstopEngaged {
                detail: "Emergency stop is engaged. Resume before sending completions.".into(),
            });
        }
        runtime::send_planner_completion_inner(prompt)
    })
    .unwrap_or_else(|e| {
        Err(FfiError::InternalPanic {
            detail: panic_detail(&e),
...
/// Returns a JSON snapshot of fallback routing state.
///
/// The payload includes the currently active provider ID and whether the
/// daemon has entered the exhausted state after all providers failed.
///
/// # Errors
///
/// Returns [`FfiError::StateError`] if the daemon is not running,
/// [`FfiError::SpawnError`] on serialisation failure,
/// or [`FfiError::InternalPanic`] if native code panics.
#[uniffi::export]
pub fn get_provider_routing_status() -> Result<String, FfiError> {
    catch_unwind(runtime::get_provider_routing_status_inner).unwrap_or_else(|e| {
        Err(FfiError::InternalPanic {
            detail: panic_detail(&e),
        })
    })
...
/// Clears the exhausted-provider latch after the UI acknowledges it.
///
/// This re-enables message sends so the next request can attempt the
/// fallback chain again from the beginning.
///
/// # Errors
///
/// Returns [`FfiError::InternalPanic`] if native code panics.
#[uniffi::export]
pub fn acknowledge_provider_exhaustion() -> Result<(), FfiError> {
    catch_unwind(|| {
        runtime::acknowledge_provider_exhaustion_inner();
        Ok(())
    })
    .unwrap_or_else(|e| {
        Err(FfiError::InternalPanic {
...
/// Returns native Piper/eSpeak phonemizer availability for this build.
///
/// This is a fail-closed readiness contract. It lets Android query the native
/// layer without assuming that GPL eSpeak or piper-phonemize binaries are
/// already bundled.
///
/// # Errors
///
/// Returns [`FfiError::InternalPanic`] if native code panics.
#[uniffi::export]
pub fn get_piper_phonemizer_status() -> Result<piper_phonemizer::PiperPhonemizerStatus, FfiError> {
    catch_unwind(|| Ok(piper_phonemizer::get_piper_phonemizer_status_inner())).unwrap_or_else(|e| {
        Err(FfiError::InternalPanic {
            detail: panic_detail(&e),
        })
    })
...
/// Sets the workspace directory path for skill operations.
///
/// Unlike the daemon config's workspace_dir, this path is available
/// immediately during app init and does not require the daemon to be
/// running. Skill listing, installation, and removal use this path
/// when the daemon is not yet started.
///
/// # Errors
///
/// Returns [`FfiError::InternalPanic`] if native code panics.
#[uniffi::export]
pub fn set_workspace_dir(path: String) -> Result<(), FfiError> {
    catch_unwind(|| {
        runtime::set_workspace_dir(std::path::PathBuf::from(path));
        Ok(())
    })
    .unwrap_or_else(|e| {
        Err(FfiError::InternalPanic {
...
/// Sets the in-app Termux bridge token for native Termux tool requests.
///
/// Android generates this token and passes it to both the Termux bridge
/// bootstrap command and the native `termux_*` tool clients. Passing `None`
/// clears the configured token and leaves the environment fallback available
/// for attach-only developer workflows.
///
/// # Errors
///
/// Returns [`FfiError::InternalPanic`] if native code panics.
#[uniffi::export]
pub fn set_termux_bridge_auth_token(token: Option<String>) -> Result<(), FfiError> {
    catch_unwind(|| {
        termux_bridge_client::set_bridge_auth_token(token);
        Ok(())
    })
    .unwrap_or_else(|e| {
        Err(FfiError::InternalPanic {
...
/// Sets the in-app Linux sandbox bridge token for native sandbox tool requests.
///
/// Android generates this token and passes it to both the `SandboxBridgeServer`
/// (NanoHTTPD on port 49481) and the native `sandbox_execute` / `sandbox_manage_process`
/// tool clients. Passing `None` clears the configured token and leaves the
/// `ZERO_ASSIST_SANDBOX_BRIDGE_TOKEN` environment fallback available.
///
/// # Errors
///
/// Returns [`FfiError::InternalPanic`] if native code panics.
#[uniffi::export]
pub fn set_sandbox_bridge_auth_token(token: Option<String>) -> Result<(), FfiError> {
    catch_unwind(|| {
        sandbox_bridge_client::set_bridge_auth_token(token);
        Ok(())
    })
    .unwrap_or_else(|e| {
        Err(FfiError::InternalPanic {
...
/// Creates a new live agent session with explicit agent-specific overrides.
///
/// Android group chat uses this to target a specific agent's provider, model,
/// credentials, and system prompt while still reusing the native session tool
/// loop and streaming callbacks.
///
/// Only one session may exist at a time; call [`session_destroy`] first if a
/// previous session is still active.
///
/// # Errors
///
/// Returns [`FfiError::StateError`] if a session is already active or the
/// daemon is not running, [`FfiError::StateCorrupted`] if the session mutex is
/// poisoned, [`FfiError::SpawnError`] if provider creation fails, or
/// [`FfiError::InternalPanic`] if native code panics.
#[uniffi::export]
pub fn session_start_custom(
    ...
    catch_unwind(AssertUnwindSafe(|| {
        session::session_start_custom_inner(
            provider_name,
            model,
            api_key,
            base_url,
...
/// [`session_send`] call. At most 200 entries are accepted; system-role
...
/// Generates a PDF via a supported third-party provider and writes it to disk.
///
/// The Kotlin layer passes a provider-specific JSON payload in the historical
/// `html` parameter. This export posts that payload to the selected provider,
/// extracts the generated PDF bytes or download URL, and writes the final file
/// to `output_path`.
///
/// # Errors
///
/// Returns [`FfiError::InvalidArgument`] for unknown providers,
/// [`FfiError::ConfigError`] for empty payloads,
/// [`FfiError::SpawnError`] for HTTP, decode, or file-write failures,
/// or [`FfiError::InternalPanic`] if native code panics.
#[uniffi::export]
pub fn generate_pdf(
    provider_name: String,
    ...
    catch_unwind(AssertUnwindSafe(|| {
        pdf_generation_ffi::generate_pdf_inner(provider_name, api_key, html, output_path)
    }))
    .unwrap_or_else(|e| {
        Err(FfiError::InternalPanic {
            detail: panic_detail(&e),
