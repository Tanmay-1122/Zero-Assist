...
pub(crate) fn send_vision_message_inner(
    ...
    let (provider_name, api_key, model) = with_daemon_config(|config| {
        let provider = config_compat::active_provider_name_or_default(config, "anthropic");
        let key = config_compat::active_api_key(config)
            .unwrap_or_default()
            .to_string();
        let mdl = config_compat::active_model_or_default(config, "claude-sonnet-4-20250514");
        (provider, key, mdl)
    })?;
