...
fn build_engine() -> Engine {
    ...
    engine.register_fn("agents", || -> Result<String, Box<EvalAltResult>> {
        let list = agents::list_agents_inner().map_err(ffi_err)?;
        Ok(list)
    });

    engine.register_fn(
        "agent_get",
        |agent_name: String| -> Result<String, Box<EvalAltResult>> {
            agents::get_agent_inner(agent_name).map_err(ffi_err)
        },
    );

    engine.register_fn(
        "delegate",
        |agent_name: String, task: String| -> Result<String, Box<EvalAltResult>> {
            agents::delegate_task_inner(agent_name, task).map_err(ffi_err)
        },
    );

    // ── Group Chat ──────────────────────────────────────────────

    engine.register_fn("group_start", || -> Result<String, Box<EvalAltResult>> {
        group_chat::start_group_chat_inner(None).map_err(ffi_err)
    });

    engine.register_fn(
        "group_start_with",
        |agents_array: rhai::Array| -> Result<String, Box<EvalAltResult>> {
            let agents: Vec<String> = agents_array
                .into_iter()
                .map(|v| v.into_string().unwrap_or_default())
                .collect();
            group_chat::start_group_chat_inner(Some(agents)).map_err(ffi_err)
        },
    );

    engine.register_fn("group_stop", || -> Result<String, Box<EvalAltResult>> {
        group_chat::stop_group_chat_inner().map_err(ffi_err)
    });

    engine.register_fn("group_status", || -> Result<String, Box<EvalAltResult>> {
        group_chat::get_group_chat_status_inner().map_err(ffi_err)
    });

    engine.register_fn(
        "group_add",
        |agent_name: String| -> Result<String, Box<EvalAltResult>> {
            group_chat::add_agent_to_group_inner(agent_name).map_err(ffi_err)
        },
    );
    ...
    engine.register_fn(
        "group_remove",
        |agent_name: String| -> Result<String, Box<EvalAltResult>> {
            group_chat::remove_agent_from_group_inner(agent_name).map_err(ffi_err)
        },
    );

    engine.register_fn(
        "group_message",
        |message: String| -> Result<String, Box<EvalAltResult>> {
            group_chat::send_group_message_inner(message).map_err(ffi_err)
        },
    );
...
mod tests {
    ...
    fn test_eval_string_literal() {
        let result =
            eval_repl_inner(r#""hello""#.into()).expect("string literal evaluation failed");
    ...
    fn test_status_returns_json() {
        ...
        let parsed: serde_json::Value =
            serde_json::from_str(&result).expect("status JSON parse failed");
        assert!(parsed.get("daemon_running").is_some());
    }

    #[test]
    fn test_validate_config_valid() {
        let result = eval_repl_inner(r#"validate_config("default_temperature = 0.7\n")"#.into())
            .expect("validate_config evaluation failed");
        assert!(
            result.is_empty(),
    ...
    fn test_validate_config_invalid() {
        let result = eval_repl_inner(r#"validate_config("not valid {{{{")"#.into())
            .expect("validate_config evaluation failed");
        assert!(
            !result.is_empty(),
    ...
    fn test_repl_models_anthropic() {
        let result =
            eval_repl_inner(r#"models("anthropic")"#.into()).expect("models evaluation failed");
        let parsed: Vec<serde_json::Value> =
            serde_json::from_str(&result).expect("models JSON parse failed");
    ...
    fn test_repl_models_with_key_anthropic() {
        let result = eval_repl_inner(r#"models_with_key("anthropic", "")"#.into())
            .expect("models_with_key evaluation failed");
        let parsed: Vec<serde_json::Value> =
            serde_json::from_str(&result).expect("models_with_key JSON parse failed");
        assert!(!parsed.is_empty());
    }
}
