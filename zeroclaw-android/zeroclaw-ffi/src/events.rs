...
#[allow(clippy::match_same_arms, clippy::too_many_lines, dead_code)]
fn event_to_kind_and_data(event: &ObserverEvent) -> (&'static str, String) {
    match event {
        ...
        ObserverEvent::ChannelMessage {
            ...
            let sender_json = sender.as_ref().map_or_else(
                || "null".to_string(),
                |s| format!(r#""{}""#, escape_json_string(s)),
            );
            let content_json = content_preview.as_ref().map_or_else(
                || "null".to_string(),
                |c| format!(r#""{}""#, escape_json_string(c)),
            );
            (
                "channel_message",
                format!(
                    r#"{{"channel":"{}","direction":"{}","sender":{},"content":{}}}"#,
        ...
        ObserverEvent::CacheHit {
            ...
            "cache_hit",
            format!(
                r#"{{"cache_type":"{}","tokens_saved":{tokens_saved}}}"#,
                escape_json_string(cache_type)
            ),
        ),
        ObserverEvent::CacheMiss { cache_type } => (
            "cache_miss",
            format!(r#"{{"cache_type":"{}"}}"#, escape_json_string(cache_type)),
        ),
        ...
        ObserverEvent::HandStarted { hand_name } => (
            "hand_started",
            format!(r#"{{"hand_name":"{}"}}"#, escape_json_string(hand_name)),
        ),
        ObserverEvent::HandCompleted {
            ...
            "hand_completed",
            format!(
                r#"{{"hand_name":"{}","duration_ms":{duration_ms},"findings_count":{findings_count}}}"#,
                escape_json_string(hand_name)
            ),
        ),
        ObserverEvent::HandFailed {
            ...
            "hand_failed",
            format!(
                r#"{{"hand_name":"{}","error":"{}","duration_ms":{duration_ms}}}"#,
                escape_json_string(hand_name),
        ...
        ObserverEvent::DeploymentStarted { deploy_id } => (
            "deployment_started",
            format!(r#"{{"deploy_id":"{}"}}"#, escape_json_string(deploy_id)),
        ),
        ObserverEvent::DeploymentCompleted {
            ...
            format!(
                r#"{{"deploy_id":"{}","commit_sha":"{}"}}"#,
                escape_json_string(deploy_id),
                escape_json_string(commit_sha)
            ),
        ),
...
mod tests {
    ...
    fn test_format_event_json_channel_message() {
        ...
        assert_eq!(parsed["data"]["sender"], "alice");
        assert_eq!(parsed["data"]["content"], "hello world");
    }

    #[test]
    fn test_format_event_json_channel_message_no_sender() {
        let event = ObserverEvent::ChannelMessage {
        ...
        assert_eq!(parsed["data"]["sender"], serde_json::json!(null));
        assert_eq!(parsed["data"]["content"], serde_json::json!(null));
    }
