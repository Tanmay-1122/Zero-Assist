...
// emitted to stderr so user redirects of stdout don't swallow it.
private const val RS = "\u001e"
private const val US = "\u001f"

// Marker emitted once at shell startup so we know bash's pid before any user
// command has finished.
private const val PID_PROBE_PREFIX = "${RS}ZEROBASHPID$US"

/**
 * Maintains a long-lived bash session inside the proot sandbox.
 *
 * Commands are serialized through an internal mutex. State (cwd, exports,
 * in-shell variables) persists between calls exactly like a real terminal.
 */
class PersistentSandboxShell(
    ...
    suspend fun run(
        ...
        ensureShell()
        // Wait for PID probe — if it doesn't arrive, shell is broken; retry once.
        if (bashPid == null) {
            withTimeoutOrNull(2.seconds) { while (bashPid == null && handle != null) delay(50.milliseconds) }
            if (bashPid == null) {
                reset()
                ensureShell()
                withTimeoutOrNull(2.seconds) { while (bashPid == null && handle != null) delay(50.milliseconds) }
        ...
        val cmdFile = File(tmpPath, ".zero_cmd_$nonce")
        try {
        ...
        val line = ". /tmp/.zero_cmd_$nonce; __zero_st=\$?; rm -f /tmp/.zero_cmd_$nonce; " +
            "printf '\\n\\036%s\\037%d\\037%d\\037%s\\036\\n' '$nonce' \"\$__zero_st\" \"\$\$\" \"\$PWD\" >&2"
    ...
    /**
     * Forward a stdin line to the running command (interactive input).
     */
     ...
     * shell itself. Signals bash's children; falls back to a full shell reset.
     */
