...
class SkillsBridge(
    ...
    /**
     * Generates and installs a skill from a natural-language description.
     *
     * Safe to call from the main thread; the underlying blocking FFI call is
     * dispatched to [ioDispatcher].
     *
     * @param description Natural-language description of the skill to generate.
     * @param author Optional author name for the generated skill metadata.
     * @return Name or path returned by the native generator.
     * @throws FfiException if the native layer reports an error.
     */
    @Throws(FfiException::class)
    suspend fun generateAndInstallSkill(
        description: String,
        author: String?,
    ): String =
        withContext(ioDispatcher) {
            com.zeroclaw.ffi.generateAndInstallSkill(description, author)
        }
