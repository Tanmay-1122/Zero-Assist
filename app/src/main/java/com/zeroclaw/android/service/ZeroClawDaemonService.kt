...
/**
 * Always-on foreground service that manages the Zero-Assist daemon lifecycle.
 *
...
class ZeroClawDaemonService : Service() {
    ...
    private var startPreparationJob: Job? = null
    private var startJob: Job? = null
    private var wakeLock: PowerManager.WakeLock? = null

    @Volatile private var isScreenOn: Boolean = true
    @Volatile private var repositoriesBound: Boolean = false
    ...
    /**
     * Binds Room-backed repositories once asynchronous application database
     * initialization has finished.
     *
     * Foreground services can be created by boot, tiles, or sticky restarts
     * before [ZeroClawApplication] has completed its background SQLCipher
     * warm-up. Reading the application's `lateinit` repositories before then
     * throws, so service start paths suspend until the readiness signal flips.
     */
    private suspend fun awaitRepositoriesReady() {
        if (repositoriesBound) return
        val app = application as ZeroClawApplication
        if (!app.repositoriesReady.value) {
            Log.d(TAG, "Waiting for repositories before daemon service startup")
            app.repositoriesReady.first { it }
        }
        bindRepositories(app)
    }

    private fun bindRepositoriesIfReady(app: ZeroClawApplication): Boolean {
        if (repositoriesBound) return true
        if (!app.repositoriesReady.value) return false
        bindRepositories(app)
        return true
    }

    @Synchronized
    private fun bindRepositories(app: ZeroClawApplication) {
        if (repositoriesBound) return
    ...
    private fun handleStartFromSettings() {
        handleStartFromSettings(restoredAfterProcessDeath = false)
    }

    private fun handleStartFromSettings(restoredAfterProcessDeath: Boolean) {
        val notification =
        ...
        startPreparationJob?.cancel()
        startPreparationJob = serviceScope.launch(ioDispatcher) {
            ...
            val configToml =
                ExternalZeroClawConfig.applyOverlay(
                    baseToml + channelsToml + agentsToml,
     ...
     *
     * Calls [ConfigValidationBridge] and logs / surfaces any validation errors,
    ...
    private suspend fun validateConfigOrStop(configToml: String): Boolean {
        try {
            val validationResult = configValidationBridge.validate(configToml)
    ...
    private fun resolveEffectiveDefaults(
        ...
        // on-device is a Kotlin-only provider — never override the Rust-facing default with it.
        if (primary.provider == "on-device") return settings
    ...
    private fun buildGlobalTomlConfig(
        ...
        SettingsTomlConfigFactory.fromSettings(
            ...
            val webDir = ExternalZeroClawConfig.webDistDir(this)
            if (java.io.File(webDir, "index.html").isFile) {
                config.copy(gatewayWebDistDir = webDir)
            } else {
                config
            }
        }
    ...
    private suspend fun buildAgentsToml(allAgents: List<Agent>): String {
        val entries =
            allAgents
                .filter { it.isEnabled && it.provider.isNotBlank() && it.modelName.isNotBlank() && it.provider != "on-device" }
                .map { agent ->
    ...
    private fun isPrimaryAgentCandidate(agent: Agent): Boolean =
        agent.isEnabled && agent.provider.isNotBlank() && agent.modelName.isNotBlank() && agent.provider != "on-device"
    ...
    private fun handleStop() {
        ...
        serviceScope.launch {
            try {
                com.zeroclaw.ffi.unregisterSharedFolderHandler()
                com.zeroclaw.ffi.unregisterDeviceControlHandler()
                bridge.stop()
                if (bindRepositoriesIfReady(application as ZeroClawApplication)) {
                    activityRepository.record(
    ...
    private fun handleStickyRestart() {
        ...
        handleStartFromSettings(restoredAfterProcessDeath = true)
    }
        ...
        startJob =
            serviceScope.launch {
                try {
                    while (true) {
                        try {
                            com.zeroclaw.ffi.registerSharedFolderHandler(
                                com.zeroclaw.android.data.saf.SharedFolderCallbackHandler(
                                    context = this@ZeroClawDaemonService,
                                    getFolderUri = {
                                        kotlinx.coroutines.runBlocking {
                                            settingsRepository.settings.first().sharedFolderUri
                                        }
                                    },
                                    getWorkflowFolderUri = {
                                        kotlinx.coroutines.runBlocking {
                                            settingsRepository.settings.first().workflowFolderUri
                                        }
                                    },
                                    workflowFolderRoot = {
                                        java.io.File(filesDir, "workflow")
                                    },
                                )
                            )
                            com.zeroclaw.ffi.registerDeviceControlHandler(
                                com.zeroclaw.android.service.devicecontrol.DeviceControlCallbackHandler(
                                    context = this@ZeroClawDaemonService,
    ...
    private fun observeNetworkState() {
        serviceScope.launch {
            networkMonitor.isConnected.collect { connected ->
                if (
                    bridge.serviceState.value == ServiceState.RUNNING &&
                    bindRepositoriesIfReady(application as ZeroClawApplication)
                ) {
                    if (!connected) {
        ...
        val receiver =
            object : BroadcastReceiver() {
                override fun onReceive(
                    ...
                    // Post to handler to return immediately and avoid ANR if main thread is blocked
                    handler.post {
                        isScreenOn = intent?.action != Intent.ACTION_SCREEN_OFF
                    }
                }
            }
        val filter =
...
typealias ZeroAssistDaemonService = ZeroClawDaemonService
