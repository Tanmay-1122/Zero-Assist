...
import com.zeroclaw.ffi.acknowledgeProviderExhaustion as ffiAcknowledgeProviderExhaustion
import com.zeroclaw.ffi.getConfiguredChannelNames
import com.zeroclaw.ffi.getProviderRoutingStatus
import com.zeroclaw.ffi.getStatus
import com.zeroclaw.ffi.scaffoldWorkspace
import com.zeroclaw.ffi.sendMessage
import com.zeroclaw.ffi.sendPlannerCompletion as ffiSendPlannerCompletion
...
class DaemonServiceBridge(
    ...
    /**
     * Optional sink for visible multi-agent chat events.
     *
     * The service layer can emit orchestration events here without depending on
     * any specific UI implementation.
     */
    @Volatile
    var agentGroupChatSink: AgentGroupChatSink? = null
    ...
    private val _currentActiveProvider = MutableStateFlow("")
    val currentActiveProvider: StateFlow<String> = _currentActiveProvider.asStateFlow()

    private val _providerExhaustionMessage = MutableStateFlow<String?>(null)
    val providerExhaustionMessage: StateFlow<String?> = _providerExhaustionMessage.asStateFlow()
    ...
    /** Current live status snapshot for visible agents across the app. */
    val agentLiveStates: StateFlow<Map<String, AgentLiveState>> = agentStatusRepository.statuses
    ...
    /**
     * Starts the Zero-Assist daemon with the provided configuration.
     *
    ...
    suspend fun start(
        ...
        val effectiveConfigToml = ExternalZeroClawConfig.applyOverlay(configToml, dataDir)
        try {
            withContext(ioDispatcher) {
                ensureStopped()
                migrateOldWorkspace()
                startDaemon(effectiveConfigToml, dataDir, host, port)
            }
            _lastError.value = null
            _serviceState.value = ServiceState.RUNNING
            _restartRequired.value = false
            refreshProviderRoutingState()
    ...
    private fun isDaemonNotRunning(detail: String?): Boolean =
        detail?.contains("daemon not running", ignoreCase = true) == true

    private fun transitionToStopped(errorDetail: String?) {
        _lastError.value = errorDetail
        _serviceState.value = ServiceState.STOPPED
        _lastStatus.value = null
        _currentActiveProvider.value = ""
        _providerExhaustionMessage.value = null
    }

    internal fun handleDaemonUnavailable(detail: String?) {
        if (isDaemonNotRunning(detail)) {
            transitionToStopped(detail)
        }
    }
    ...
    suspend fun stop() {
        ...
        try {
            ...
            _lastError.value = null
            transitionToStopped(errorDetail = null)
        } catch (e: FfiException) {
            if (isDaemonNotRunning(e.errorDetail())) {
                transitionToStopped(errorDetail = null)
                return
            }
            _lastError.value = e.errorDetail()
     ...
     *
     * **Performance:** Adds timing logs when duration exceeds threshold.
     *
    ...
    suspend fun pollStatus(): DaemonStatus {
        ...
        return try {
            ...
            if (status.running) {
                _lastError.value = null
                _serviceState.value = ServiceState.RUNNING
            } else if (_serviceState.value == ServiceState.RUNNING) {
                transitionToStopped(errorDetail = "daemon not running")
            }

            val duration = System.currentTimeMillis() - startTime
            if (duration > SLOW_POLL_THRESHOLD_MS) {
                Log.w(TAG, "FFI pollStatus() took ${duration}ms (expected <10ms)")
            }

            status
        } catch (e: FfiException) {
            val duration = System.currentTimeMillis() - startTime
            Log.w(TAG, "FFI pollStatus() failed after ${duration}ms")
            handleDaemonUnavailable(e.errorDetail())
            throw e
        }
    }
     ...
     *
     * **Performance:** Adds timing logs to track FFI call duration.
     *
    ...
    suspend fun send(message: String): String {
        val startTime = System.currentTimeMillis()
        return try {
            val result = withContext(ioDispatcher) { sendMessage(message) }
            val duration = System.currentTimeMillis() - startTime
            Log.d(TAG, "FFI send() completed in ${duration}ms")
            result
        } catch (e: FfiException) {
            val duration = System.currentTimeMillis() - startTime
            Log.w(TAG, "FFI send() failed after ${duration}ms: ${e.errorDetail()}")
            val detail = e.errorDetail()
            handleDaemonUnavailable(detail)
            if (detail.contains(ALL_PROVIDERS_EXHAUSTED_MESSAGE, ignoreCase = true) ||
                detail.contains("All providers/models failed", ignoreCase = true)
            ) {
                _providerExhaustionMessage.value = ALL_PROVIDERS_EXHAUSTED_MESSAGE
            }
    ...
    /**
     * One-shot LLM completion for internal planners (device-control).
     *
     * Bypasses the full agent loop — no tools, memory, observers, or
     * conversation history. Calls `chat_with_system` directly on the
     * configured provider. This avoids building a full tool registry
     * and agent loop for each planner step.
     *
     * Safe to call from the main thread. The underlying **blocking** FFI
     * call is dispatched to [ioDispatcher].
     *
     * **Performance:** Adds timing logs to track FFI call duration.
     *
     ...
     */
    @Throws(FfiException::class)
    suspend fun sendPlannerCompletion(prompt: String): String {
        val startTime = System.currentTimeMillis()
        return try {
            val result = withContext(ioDispatcher) { ffiSendPlannerCompletion(prompt) }
            val duration = System.currentTimeMillis() - startTime
            Log.d(TAG, "FFI sendPlannerCompletion() completed in ${duration}ms")
            result
        } catch (e: FfiException) {
            val duration = System.currentTimeMillis() - startTime
            Log.w(TAG, "FFI sendPlannerCompletion() failed after ${duration}ms")
            val detail = e.errorDetail()
            handleDaemonUnavailable(detail)
            if (detail.contains(ALL_PROVIDERS_EXHAUSTED_MESSAGE, ignoreCase = true) ||
                detail.contains("All providers/models failed", ignoreCase = true)
     ...
     *
     * Creates the standard `Zero-Assist` workspace structure (5 subdirectories
     ...
     *
     * @param agentName Name for the AI agent (empty defaults to "Zero-Assist").
    ...
    @Throws(FfiException::class)
    suspend fun refreshProviderRoutingState() {
        val json = withContext(ioDispatcher) { getProviderRoutingStatus() }
        val state = JSONObject(json)
        _currentActiveProvider.value =
        if (state.isNull("active_provider")) {
            ""
        } else {
            state.optString("active_provider").orEmpty()
        }
        if (state.optBoolean("master_ai_exhausted", false) || state.optBoolean("exhausted", false)) {
            _providerExhaustionMessage.value = ALL_PROVIDERS_EXHAUSTED_MESSAGE
        }
    }

    @Throws(FfiException::class)
    suspend fun acknowledgeProviderExhaustion() {
        withContext(ioDispatcher) { ffiAcknowledgeProviderExhaustion() }
        _providerExhaustionMessage.value = null
    }
    ...
    /**
     * Updates the visible live state for a single agent.
     */
    fun updateAgentLiveState(
        agentId: String,
        status: AgentStatus,
        currentTask: String = "",
    ) {
    ...
    /**
     * Resets an agent back to the default idle state.
     */
    fun resetAgentLiveState(agentId: String) {
        updateAgentLiveState(agentId = agentId, status = AgentStatus.IDLE)
    }

    fun clearAgentLiveState(agentId: String) {
        agentStatusRepository.clearStatus(agentId)
    }

    fun markAgentExecuting(
        agentId: String,
        task: String = "",
    ) {
        updateAgentLiveState(agentId = agentId, status = AgentStatus.EXECUTING, currentTask = task)
    }

    fun markAgentWaiting(
        agentId: String,
        task: String = "",
    ) {
        updateAgentLiveState(agentId = agentId, status = AgentStatus.WAITING, currentTask = task)
    }
    ...
    fun markAgentError(
        ...
        updateAgentLiveState(agentId = agentId, status = AgentStatus.ERROR, currentTask = task)
    }

    /**
     * Emits a visible task assignment summary for an inter-agent handoff.
     *
     * The daemon keeps raw orchestration data private; the UI only receives a
     * short human-readable summary capped to two sentences.
     */
    fun emitAgentAssigning(fromAgent: Agent, toAgent: Agent, taskSummary: String) {
        val visibleSummary =
            VisibleAgentChatFormatter.taskAssignmentSummary(
                fromAgentName = fromAgent.name,
                toAgentName = toAgent.name,
                rawSummary = taskSummary,
            )
        updateAgentLiveState(
            agentId = fromAgent.id,
            status = AgentStatus.DELEGATING,
            currentTask = visibleSummary,
        )
        agentGroupChatSink?.onAgentAssigning(fromAgent, toAgent, visibleSummary)
    }

    /**
     * Emits a visible one-sentence status update for an agent.
     */
    fun emitAgentStatusUpdate(agent: Agent, status: String) {
        val visibleStatus =
            VisibleAgentChatFormatter.statusUpdate(
                agentName = agent.name,
                rawStatus = status,
            )
        val existingStatus = agentStatusRepository.getStatus(agent.id)?.status
        updateAgentLiveState(
            agentId = agent.id,
            status =
                when (existingStatus) {
            ...
            currentTask = visibleStatus,
        )
        agentGroupChatSink?.onAgentStatusUpdate(agent, visibleStatus)
    }

    /**
     * Emits a visible result summary capped to three sentences.
     */
    fun emitAgentResult(agent: Agent, summary: String) {
        val visibleSummary =
            VisibleAgentChatFormatter.resultSummary(
                agentName = agent.name,
                rawSummary = summary,
            )
        updateAgentLiveState(
        ...
        agentGroupChatSink?.onAgentResult(agent, visibleSummary)
    }

    fun emitAgentTypingStart(agentId: String) {
        val currentTask = agentStatusRepository.getStatus(agentId)?.currentTask.orEmpty()
        updateAgentLiveState(
            agentId = agentId,
            status = AgentStatus.THINKING,
            currentTask = currentTask,
        )
        agentGroupChatSink?.onAgentTypingStart(agentId)
    }

    fun emitAgentTypingStop(agentId: String) {
        val existing = agentStatusRepository.getStatus(agentId)
        if (existing?.status == AgentStatus.THINKING) {
            resetAgentLiveState(agentId)
        }
        agentGroupChatSink?.onAgentTypingStop(agentId)
    }

    fun emitAgentStreamingChunk(messageId: String, chunk: String) {
        val visibleChunk = VisibleAgentChatFormatter.streamingChunk(chunk)
        if (visibleChunk.isEmpty()) return
        agentGroupChatSink?.onAgentStreamingChunk(messageId, visibleChunk)
    }
    ...
    suspend fun hotSwapProvider(
        ...
        withContext(ioDispatcher) {
            try {
                ...
                _restartRequired.value = false
                _currentActiveProvider.value = provider
                true
            } catch (e: Exception) {
    ...
    companion object {
        ...
        private const val ALL_PROVIDERS_EXHAUSTED_MESSAGE =
            "All providers exhausted. Please check your API keys or limits."

        /** Maximum length for workspace identity strings passed to FFI. */
        private const val MAX_IDENTITY_LENGTH = 100

        /** Threshold in milliseconds for logging slow pollStatus() calls. */
        private const val SLOW_POLL_THRESHOLD_MS = 50L
    }
}
...
private fun FfiException.errorDetail(): String =
    when (this) {
        ...
        is FfiException.GenerationException -> detail
    }
