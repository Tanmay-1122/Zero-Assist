...
/**
 * Runs diagnostic checks against the Zero-Assist configuration, API keys,
 ...
 * @param configValidationBridge Boundary for ZeroClaw Master config validation.
 */
class DoctorValidator(
    ...
    private val configValidationBridge: ConfigValidationBridge = NativeConfigValidationBridge,
    private val termuxRuntimeStatusProvider: TermuxRuntimeStatusProvider? = null,
) {
    /**
     * Validates each agent's TOML config individually and the combined config.
     *
     * Uses [ConfigValidationBridge] which parses TOML without
     * starting the daemon.
     *
    ...
    /**
     * Checks readiness of the local Termux bridge used by first-party Termux tools.
     */
    suspend fun runTermuxChecks(): List<DiagnosticCheck> {
        val provider =
            termuxRuntimeStatusProvider
                ?: return listOf(
                    DiagnosticCheck(
                        id = "termux-unconfigured",
                        category = DiagnosticCategory.TERMUX,
                        title = "Termux diagnostics",
                        status = CheckStatus.WARN,
                        detail = "Termux diagnostics are not configured in this app build.",
                    ),
                )
        ...
        val status =
            try {
                ...
                return listOf(
                    DiagnosticCheck(
                        id = "termux-status-error",
                        category = DiagnosticCategory.TERMUX,
                        title = "Termux diagnostics",
                        status = CheckStatus.FAIL,
                        detail = "Failed to inspect Termux runtime: ${e.message ?: e::class.java.simpleName}.",
                    ),
                )
            }

        return listOf(
            termuxPackageCheck(status),
            termuxPermissionCheck(status),
            termuxBootstrapCheck(status),
            termuxBridgeHealthCheck(status),
            termuxProotCheck(status),
        )
    }

    private fun termuxPackageCheck(status: TermuxRuntimeStatus): DiagnosticCheck {
        val packageState = status.packageState
        return DiagnosticCheck(
            id = "termux-package",
            category = DiagnosticCategory.TERMUX,
            title = "Termux package",
            status =
                when (packageState.availability) {
                    TermuxPackageAvailability.INSTALLED -> CheckStatus.PASS
                    TermuxPackageAvailability.NOT_VISIBLE -> CheckStatus.WARN
                    TermuxPackageAvailability.NOT_INSTALLED -> CheckStatus.FAIL
                },
            detail =
                when (packageState.availability) {
                    TermuxPackageAvailability.INSTALLED ->
                        "Termux is installed${packageState.versionName?.let { " ($it)" }.orEmpty()}."
                    TermuxPackageAvailability.NOT_VISIBLE ->
                        "Android package visibility did not expose Termux. Confirm com.termux is installed."
    ...
    private fun termuxPermissionCheck(status: TermuxRuntimeStatus): DiagnosticCheck {
        val permissionState = status.permissionState
        return DiagnosticCheck(
            id = "termux-run-command-permission",
            category = DiagnosticCategory.TERMUX,
            title = "RUN_COMMAND permission",
            status =
                when (permissionState.availability) {
                    TermuxPermissionAvailability.GRANTED -> CheckStatus.PASS
                    TermuxPermissionAvailability.UNKNOWN -> CheckStatus.WARN
                    TermuxPermissionAvailability.DENIED -> CheckStatus.FAIL
                },
            detail =
                when (permissionState.availability) {
                    TermuxPermissionAvailability.GRANTED ->
                        "Zero-Assist can request Termux RUN_COMMAND execution."
                    TermuxPermissionAvailability.UNKNOWN ->
                        "RUN_COMMAND permission could not be verified on this Android version."
                    TermuxPermissionAvailability.DENIED ->
                        "Grant Zero-Assist the Termux RUN_COMMAND permission and keep allow-external-apps enabled in Termux."
                },
        )
    }

    private fun termuxBootstrapCheck(status: TermuxRuntimeStatus): DiagnosticCheck {
        val bootstrapState = status.bootstrapState
        return DiagnosticCheck(
            id = "termux-run-command-service",
            category = DiagnosticCategory.TERMUX,
            title = "RUN_COMMAND service",
            status =
                when (bootstrapState.availability) {
                    TermuxBootstrapAvailability.AVAILABLE -> CheckStatus.PASS
                    TermuxBootstrapAvailability.UNKNOWN -> CheckStatus.WARN
                    TermuxBootstrapAvailability.UNAVAILABLE -> CheckStatus.FAIL
                },
            detail =
                when (bootstrapState.availability) {
                    TermuxBootstrapAvailability.AVAILABLE ->
                        "Termux exposes the public RunCommandService used to start the bridge."
                    TermuxBootstrapAvailability.UNKNOWN ->
                        "Termux RunCommandService availability is unknown."
                    TermuxBootstrapAvailability.UNAVAILABLE ->
                        "Termux RunCommandService is unavailable. Update Termux and enable external app commands."
                },
        )
    }

    private fun termuxBridgeHealthCheck(status: TermuxRuntimeStatus): DiagnosticCheck =
        DiagnosticCheck(
            id = "termux-bridge-health",
            category = DiagnosticCategory.TERMUX,
            title = "Zero-Assist Termux bridge",
            status =
                when {
                    status.health.status == TermuxHealthStatus.READY -> CheckStatus.PASS
            ...
            detail =
                buildString {
                    append(status.health.reason.ifBlank { status.inactiveReason() })
                    status.health.details.endpoint?.let { append(" Endpoint: $it.") }
                    status.health.details.workspace?.let { append(" Workspace: $it.") }
                    status.health.details.version?.let { append(" Bridge: $it.") }
                },
        )

    private fun termuxProotCheck(status: TermuxRuntimeStatus): DiagnosticCheck {
        val proot = status.health.details.proot
        val available = proot.available == true
        val distros = proot.distros.joinToString().takeIf { it.isNotBlank() }
        return DiagnosticCheck(
            id = "termux-proot",
            category = DiagnosticCategory.TERMUX,
            title = "proot-distro",
            ...
            detail =
                when {
                    available && distros != null -> "proot-distro is available. Distros: $distros."
                    available -> "proot-distro is available."
                    else -> "proot-distro is optional and is not available yet. Install it in Termux before using distro-backed tools."
                },
        )
    }
    ...
    private suspend fun checkAgentConfig(agent: Agent): DiagnosticCheck {
        ...
        val result = withContext(ioDispatcher) { configValidationBridge.validate(toml) }
     ...
     * @return TOML string suitable for [ConfigValidationBridge].
     */
