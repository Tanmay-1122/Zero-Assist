...
class DeviceControlAccessibilityService : AccessibilityService(), DeviceControlServiceBridge {
     ...
     * to compute a [ScreenFingerprint] without collecting full node metadata.
     * Roughly 3× faster than a full [snapshot] call.
     */
    override fun snapshotFingerprint(): ScreenFingerprint {
        ...
        try {
            roots.forEach { root ->
                traverseForFingerprint(root) { text, clickable, editable ->
                    nodeCount++
                    if (editable) editableCount++
                    hashAccum = hashAccum * 31 + text.hashCode() + (if (clickable) 1 else 0)
                }
            }
        } finally {
            roots.forEach { it.recycle() }
        ...
        return ScreenFingerprint(
            contentHash = hashAccum,
            packageName = pkg ?: "",
            actionableNodeCount = nodeCount,
            hasEditableField = editableCount > 0,
            uniqueLabelCount = nodeCount,
        )
    }

    private fun traverseForFingerprint(
        node: AccessibilityNodeInfo,
        visitor: (text: String, clickable: Boolean, editable: Boolean) -> Unit,
    ) {
        ...
        if (node.isVisibleToUser && !bounds.isEmpty) {
            ...
            if (label.isNotBlank() || node.isClickable || node.isEditable) {
                visitor(label, node.isClickable, node.isEditable)
            }
        }
        for (i in 0 until node.childCount) {
            node.getChild(i)?.let { child ->
                try { traverseForFingerprint(child, visitor) } finally { child.recycle() }
            }
        }
    }
