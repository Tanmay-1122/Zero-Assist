...
import com.zeroclaw.android.ui.theme.PureBlack
    ...
    Canvas(modifier = Modifier.fillMaxSize()) {
        ...
        drawCircle(
            brush = Brush.radialGradient(
                colors = listOf(AccentBlue.copy(alpha = GLOB1_ALPHA), Color.Transparent),
                center = glob1Pos,
        ...
        drawCircle(
            brush = Brush.radialGradient(
                colors = listOf(AccentBlue.copy(alpha = GLOB2_ALPHA), Color.Transparent),
                center = glob2Pos,
        ...
        drawRect(
            brush = Brush.radialGradient(
                colors = listOf(Color.Transparent, PureBlack.copy(alpha = VIGNETTE_ALPHA)),
                center = center,
                radius = width
            )
        )
...
private const val TRANSITION_DURATION_MS = 4000
