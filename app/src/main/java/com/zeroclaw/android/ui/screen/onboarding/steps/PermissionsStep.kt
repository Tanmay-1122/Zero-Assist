...
fun PermissionsStep(
    ...
    val cameraAvailable = remember(context) { CameraBridge.hasCameraHardware(context) }
    ...
    Column {
        ...
        Text(
            text =
                "Zero-Assist needs a few permissions to run " +
        ...
        if (cameraAvailable) {
            ...
            Text(
                text = "Required for voice commands that control the flashlight.",
                style = MaterialTheme.typography.bodyMedium,
                color = MaterialTheme.colorScheme.onSurfaceVariant,
            )
            Spacer(modifier = Modifier.height(8.dp))
            if (cameraGranted) {
                Row(
                    ...
                    Icon(
                        imageVector = Icons.Filled.CheckCircle,
                        contentDescription = "Camera permission granted",
                        tint = MaterialTheme.colorScheme.primary,
                        modifier = Modifier.size(20.dp),
                    )
                    Text(
        ...
        Text(
            text =
                "Lets Zero-Assist show a live status overlay while it " +
                    "controls the device on your behalf.",
            style = MaterialTheme.typography.bodyMedium,
            color = MaterialTheme.colorScheme.onSurfaceVariant,
        )
        Spacer(modifier = Modifier.height(8.dp))
