 ...
 * @property callId Monotonic counter assigned on the Kotlin side when [onToolStart] fires.
 *                  Used to catch downstream mismatches between tool start and result.
 */
@Immutable
 ...
 * @property hint Short argument summary from the tool start event.
 * @property callId Monotonic counter matching the originating [ToolProgress].
 * @property canvasFrame Parsed canvas frame when the tool is a canvas render.
 */
@Immutable
data class ToolResultEntry(
    ...
    val hint: String = "",
    val callId: Long = 0L,
    val canvasFrame: CanvasFrame? = null,
)
