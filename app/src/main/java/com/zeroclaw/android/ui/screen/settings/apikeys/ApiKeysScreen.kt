 ...
 * @param onNavigateToOnDeviceModels Callback to navigate to the on-device models screen.
 * @param onOnDeviceSelected Callback when the user selects on-device AI as the active connection.
...
fun ApiKeysScreen(
    ...
    val onDeviceEngineState by apiKeysViewModel.onDeviceEngineState.collectAsStateWithLifecycle()
    val onDeviceLoadedModelId by apiKeysViewModel.onDeviceLoadedModelId.collectAsStateWithLifecycle()
    ...
    ApiKeysContent(
        ...
        onDeviceLoadedModelId = onDeviceLoadedModelId,
...
internal fun ApiKeysContent(
    ...
    onDeviceLoadedModelId: String? = null,
    onNavigateToDetail: (String?) -> Unit,
    onRequestBiometric: (keyId: String) -> Unit,
    ...
    Scaffold(
        ...
        // Always show the on-device card + lazy list together — the on-device
        // option is always present regardless of whether any API keys are stored.
        LazyColumn(
            modifier =
                Modifier
                    .fillMaxSize()
            ...
            // ── On-Device AI card (always at top) ──────────────────────────────
            item(key = "on_device_card", contentType = "on_device") {
                OnDeviceConnectionCard(
                    engineState = onDeviceEngineState,
                    loadedModelId = onDeviceLoadedModelId,
                    onManageModels = onNavigateToOnDeviceModels,
                    onSelect = onOnDeviceSelected,
                )
            }
...
/**
 * Connection card for the on-device LiteRT inference engine.
 *
 * Renders a premium-styled card that:
 * - Shows a coloured status badge reflecting the current [EngineState]
 * - Displays the currently loaded model name when ready
 * - Offers a "Manage Models" action to navigate to [LiteRTModelsScreen]
 * - Can be clicked to select on-device AI as the active connection
 *
 * @param engineState Current lifecycle state of the local inference engine.
 * @param loadedModelId Currently loaded model ID, or null if none.
 * @param onManageModels Callback to open the model management screen.
 * @param onSelect Callback when the user selects on-device AI as the connection.
 * @param modifier Modifier applied to the card.
 */
@Composable
private fun OnDeviceConnectionCard(
    ...
    val statusLabel = when (engineState) {
        EngineState.READY -> loadedModelId?.let { "Ready — $it" } ?: "Ready"
    ...
    OutlinedCard(
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            ...
            Text(
                text = "Runs entirely on your device — no API key or internet required. " +
                    "Supports Gemini Nano, Phi-3 Mini, LLaMA 3.2, and more.",
                style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant,
            )
            ...
            OutlinedButton(
                ...
                modifier = Modifier
                    .semantics { contentDescription = "Manage on-device models" },
