...
interface SettingsRepository {
    ...
    /**
     * Updates the maximum tool-call iterations per agent turn.
     *
     * @param max Maximum iterations (0 = use Rust default of 10, 99999 = unlimited).
     */
    suspend fun setMaxToolIterations(max: Int)
    ...
    /**
     * Updates the number of retries against the selected provider.
     *
    ...
    /**
     * Updates the deprecated fallback provider list for stored settings compatibility.
     *
    ...
    /**
     * Updates the deprecated fallback provider configuration JSON for stored settings compatibility.
     *
     * @param json JSON array of fallback provider objects.
     */
    suspend fun setFallbackProviderConfigsJson(json: String)
    ...
    /**
     * Updates the web distribution directory path for the gateway static files.
     *
     * @param dir Absolute path to the web/dist directory.
     */
    suspend fun setGatewayWebDistDir(dir: String)
    ...
    /**
     * Toggles WebAuthn/passkey authentication support.
     *
     * @param enabled Whether WebAuthn is enabled.
     */
    suspend fun setSecurityWebauthnEnabled(enabled: Boolean)

    /**
     * Updates the WebAuthn relying-party ID.
     *
     * @param rpId RP ID (domain/host).
     */
    suspend fun setSecurityWebauthnRpId(rpId: String)

    /**
     * Updates the WebAuthn relying-party origin.
     *
     * @param rpOrigin RP origin URL.
     */
    suspend fun setSecurityWebauthnRpOrigin(rpOrigin: String)

    /**
     * Updates the WebAuthn relying-party display name.
     *
     * @param rpName RP display name.
     */
    suspend fun setSecurityWebauthnRpName(rpName: String)
    ...
    /**
     * Toggles automatic terminal task delegation.
     *
     * @param enabled Whether terminal prompts should be auto-routed to the best agent.
     */
    suspend fun setTerminalAutoDelegateEnabled(enabled: Boolean)

    /**
     * Stores the user's wake-word preference without starting background listening.
     *
     * @param enabled Whether wake-word activation should be used once local detector support exists.
     */
    suspend fun setVoiceWakeupRequested(enabled: Boolean)
    ...
    /**
     * Toggles the shared folder feature.
     *
     * @param enabled Whether the shared folder is enabled.
     */
    suspend fun setSharedFolderEnabled(enabled: Boolean)

    /**
     * Updates the persistent SAF URI to the shared folder.
     *
    ...
    /**
     * Toggles the default workflow folder feature.
     *
     * @param enabled Whether the workflow folder is enabled.
     */
    suspend fun setWorkflowFolderEnabled(enabled: Boolean)

    /**
     * Updates the optional persistent SAF URI to the workflow folder.
     *
     * @param uri The SAF Tree URI, or blank to use the app-owned default workflow folder.
     */
    suspend fun setWorkflowFolderUri(uri: String)

    /**
     * Toggles the Linux Sandbox plugin (Alpine Linux via proot).
     *
     * @param enabled Whether the sandbox is enabled.
     */
    suspend fun setLinuxSandboxEnabled(enabled: Boolean)

    /**
     * Sets the maximum timeout for sandbox execute commands in seconds.
     *
     * @param secs Timeout in seconds (default is 10800, which is 3 hours).
     */
    suspend fun setLinuxSandboxTimeoutSecs(secs: Int)

    /**
     * Toggles the Google Workspace plugin.
     *
     * @param enabled Whether Google Workspace integration is active.
     */
    suspend fun setGoogleWorkspaceEnabled(enabled: Boolean)

    /** Sets the comma-separated list of allowed Google Workspace services. */
    suspend fun setGoogleWorkspaceAllowedServices(services: String)

    /** Sets whether Google Workspace API calls are audit-logged. */
    suspend fun setGoogleWorkspaceAuditLog(enabled: Boolean)

    /** Toggles the Termux plugin. */
    suspend fun setTermuxEnabled(enabled: Boolean)

    /** Sets the Termux permission tier ("MEDIUM", "HIGH", or "UNCONSTRAINED"). */
    suspend fun setTermuxPermissionTier(tier: String)

    /** Sets whether to show terminal command log in plugin detail. */
    suspend fun setTermuxShowTerminalLog(show: Boolean)

    /** Sets pre-approved command patterns as comma-separated string. */
    suspend fun setTermuxPreApprovedPatterns(patterns: String)

    /** Toggles external MCP tool loading. */
    suspend fun setMcpEnabled(enabled: Boolean)

    /** Toggles on-demand MCP tool schema loading via tool_search. */
    suspend fun setMcpDeferredLoading(enabled: Boolean)

    /** Updates the JSON-serialized list of MCP server entries. */
