...
class RoomTerminalEntryRepository(
    ...
    scrollbackLimit: Int = DEFAULT_TERMINAL_SCROLLBACK_LIMIT,
) : TerminalEntryRepository {
    private val boundedScrollbackLimit = scrollbackLimit.coerceAtLeast(MIN_TERMINAL_SCROLLBACK_LIMIT)

    override val entries: Flow<List<TerminalEntry>> =
        dao.observeRecent(boundedScrollbackLimit).map { entities ->
    ...
    companion object {
        internal const val DEFAULT_TERMINAL_SCROLLBACK_LIMIT = 180
        private const val MIN_TERMINAL_SCROLLBACK_LIMIT = 1
    }
}
