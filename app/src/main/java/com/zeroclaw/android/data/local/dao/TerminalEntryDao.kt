...
interface TerminalEntryDao {
    ...
    /**
     * Observes the most recent terminal entries, returned oldest first for display.
     */
    @Query(
        """
        SELECT * FROM (
            SELECT * FROM terminal_entries
    ...
    fun observeRecent(limit: Int): Flow<List<TerminalEntryEntity>>
