...
object ProviderRegistry {
    ...
    /**
     * On-device inference providers that require no API key or internet access.
     *
     * The "on-device" provider is a virtual entry backed by [LocalInferenceEngine]
     * (LiteRT). It has no network credentials and is always present in the list,
     * with availability determined at runtime by the engine state.
     */
    private fun onDeviceProviders(): List<ProviderInfo> =
        listOf(
            ProviderInfo(
                id = "on-device",
                displayName = "On-Device AI",
                authType = ProviderAuthType.ON_DEVICE,
                category = ProviderCategory.ON_DEVICE,
                ...
                helpText = "Runs entirely on your device. No API key or internet required.",
            ),
        )
    ...
    private fun ecosystemProviders(): List<ProviderInfo> =
        listOf(
            ...
            ProviderInfo(
                ...
                aliases = listOf("qwen2.5", "qwen2.5-72b", "qwen2.5-32b"),
                category = ProviderCategory.ECOSYSTEM,
                iconUrl = faviconUrl("qwen.ai"),
                modelListUrl = "https://api.qwen.ai/v1/models",
                modelListFormat = ModelListFormat.OPENAI_COMPATIBLE,
                keyCreationUrl = "https://qwen.ai/account/api",
                keyPrefix = "qwen-",
                keyPrefixHint = "Keys start with qwen-",
                helpText = "Qwen models from Alibaba Cloud / Qwen.ai",
            ),
            ProviderInfo(
                id = "deepseek",
