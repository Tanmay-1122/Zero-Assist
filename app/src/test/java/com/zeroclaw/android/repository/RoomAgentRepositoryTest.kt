...
class RoomAgentRepositoryTest {
    ...
    fun setUp() {
        ...
        syncRepository = mockk(relaxUnitFun = true)
        every { database.agentDao() } returns dao
        every { dao.observeAll() } returns flowOf(emptyList())
        every { dao.observeMasterAgent() } returns flowOf(null)
        every { dao.observeActiveAgentsByPriority() } returns flowOf(emptyList())
        repo = RoomAgentRepository(database, syncRepository)
    }
    ...
    fun `save delegates to dao upsert`() =
        runTest {
            ...
            coVerify { dao.upsertAgent(agent.toEntity()) }
            coVerify { syncRepository.markPendingSync() }
        }
