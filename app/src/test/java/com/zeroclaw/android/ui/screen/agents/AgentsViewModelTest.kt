...
private class TestAgentRepository(
    ...
    override val masterAgent = _agents.map { current -> current.firstOrNull { it.isMaster } }
    override val activeAgentsByPriority = _agents.map { current ->
        current.filter { it.isEnabled }.sortedByDescending { it.priority }
    }
    ...
    override fun observeByRole(role: AgentRole) =
        _agents.map { current -> current.filter { it.role == role } }
}
