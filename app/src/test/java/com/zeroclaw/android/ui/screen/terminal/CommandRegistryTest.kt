...
class CommandRegistryTest {
    ...
    inner class ParseAndTranslate {
        ...
        @Test
        @DisplayName("routes device smoke to LocalAction")
        fun `parseAndTranslate routes device smoke to LocalAction`() {
            val result = CommandRegistry.parseAndTranslate("/device smoke")
            assertTrue(result is CommandResult.LocalAction)
            assertEquals("device smoke", (result as CommandResult.LocalAction).action)
        }

        @Test
        @DisplayName("routes device status to LocalAction")
        fun `parseAndTranslate routes device status to LocalAction`() {
            val result = CommandRegistry.parseAndTranslate("/device status")
            assertTrue(result is CommandResult.LocalAction)
            assertEquals("device status", (result as CommandResult.LocalAction).action)
        }

        @Test
        @DisplayName("routes termux status to LocalAction")
        fun `parseAndTranslate routes termux status to LocalAction`() {
            val result = CommandRegistry.parseAndTranslate("/termux status")
            assertTrue(result is CommandResult.LocalAction)
            assertEquals("termux status", (result as CommandResult.LocalAction).action)
        }

        @Test
        @DisplayName("routes termux start to LocalAction")
        fun `parseAndTranslate routes termux start to LocalAction`() {
            val result = CommandRegistry.parseAndTranslate("/termux start")
            assertTrue(result is CommandResult.LocalAction)
            assertEquals("termux start", (result as CommandResult.LocalAction).action)
        }

        @Test
        @DisplayName("routes termux setup to LocalAction")
        fun `parseAndTranslate routes termux setup to LocalAction`() {
            val result = CommandRegistry.parseAndTranslate("/termux setup")
            assertTrue(result is CommandResult.LocalAction)
            assertEquals("termux setup", (result as CommandResult.LocalAction).action)
        }

        @Test
        @DisplayName("routes termux capabilities to LocalAction")
        fun `parseAndTranslate routes termux capabilities to LocalAction`() {
            val result = CommandRegistry.parseAndTranslate("/termux capabilities")
            assertTrue(result is CommandResult.LocalAction)
            assertEquals("termux capabilities", (result as CommandResult.LocalAction).action)
        }

        @Test
        @DisplayName("routes termux smoke to LocalAction")
        fun `parseAndTranslate routes termux smoke to LocalAction`() {
            val result = CommandRegistry.parseAndTranslate("/termux smoke")
            assertTrue(result is CommandResult.LocalAction)
            assertEquals("termux smoke", (result as CommandResult.LocalAction).action)
        }

        @Test
        @DisplayName("routes device doctor to LocalAction")
        fun `parseAndTranslate routes device doctor to LocalAction`() {
            val result = CommandRegistry.parseAndTranslate("/device doctor")
            assertTrue(result is CommandResult.LocalAction)
            assertEquals("device doctor", (result as CommandResult.LocalAction).action)
        }

        @Test
        @DisplayName("routes device whatsapp to LocalAction with raw args")
        fun `parseAndTranslate routes device whatsapp to LocalAction with raw args`() {
            val result =
                CommandRegistry.parseAndTranslate(
                    "/device whatsapp Sweetheart :: zero assist check",
                )

            assertTrue(result is CommandResult.LocalAction)
            result as CommandResult.LocalAction
            assertEquals("device whatsapp", result.action)
            assertEquals("Sweetheart :: zero assist check", result.rawArgsText)
        }
