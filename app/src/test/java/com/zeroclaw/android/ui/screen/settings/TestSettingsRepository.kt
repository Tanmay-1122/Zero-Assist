...
internal class TestSettingsRepository : SettingsRepository {
    ...
    override suspend fun setMaxToolIterations(max: Int) = _settings.update { it.copy(maxToolIterations = max) }
    ...
    override suspend fun setFallbackProviderConfigsJson(json: String) = _settings.update { it.copy(fallbackProviderConfigsJson = json) }
    ...
    override suspend fun setGatewayWebDistDir(dir: String) = _settings.update { it.copy(gatewayWebDistDir = dir) }
    ...
    override suspend fun setSecurityWebauthnEnabled(enabled: Boolean) = _settings.update { it.copy(securityWebauthnEnabled = enabled) }

    override suspend fun setSecurityWebauthnRpId(rpId: String) = _settings.update { it.copy(securityWebauthnRpId = rpId) }

    override suspend fun setSecurityWebauthnRpOrigin(rpOrigin: String) = _settings.update { it.copy(securityWebauthnRpOrigin = rpOrigin) }

    override suspend fun setSecurityWebauthnRpName(rpName: String) = _settings.update { it.copy(securityWebauthnRpName = rpName) }
    ...
    override suspend fun setTerminalAutoDelegateEnabled(enabled: Boolean) =
        _settings.update { it.copy(terminalAutoDelegateEnabled = enabled) }

    override suspend fun setVoiceWakeupRequested(enabled: Boolean) =
        _settings.update { it.copy(voiceWakeupRequested = enabled) }
    ...
    override suspend fun setWorkflowFolderEnabled(enabled: Boolean) =
        _settings.update { it.copy(workflowFolderEnabled = enabled) }

    override suspend fun setWorkflowFolderUri(uri: String) =
        _settings.update { it.copy(workflowFolderUri = uri) }
    ...
    override suspend fun setLinuxSandboxEnabled(enabled: Boolean) =
        _settings.update { it.copy(linuxSandboxEnabled = enabled) }

    override suspend fun setLinuxSandboxTimeoutSecs(secs: Int) =
        _settings.update { it.copy(linuxSandboxTimeoutSecs = secs) }

    override suspend fun setGoogleWorkspaceEnabled(enabled: Boolean) =
        _settings.update { it.copy(googleWorkspaceEnabled = enabled) }

    override suspend fun setGoogleWorkspaceAllowedServices(services: String) =
        _settings.update { it.copy(googleWorkspaceAllowedServices = services) }

    override suspend fun setGoogleWorkspaceAuditLog(enabled: Boolean) =
        _settings.update { it.copy(googleWorkspaceAuditLog = enabled) }

    override suspend fun setTermuxEnabled(enabled: Boolean) =
        _settings.update { it.copy(termuxEnabled = enabled) }

    override suspend fun setTermuxPermissionTier(tier: String) =
        _settings.update { it.copy(termuxPermissionTier = tier) }

    override suspend fun setTermuxShowTerminalLog(show: Boolean) =
        _settings.update { it.copy(termuxShowTerminalLog = show) }

    override suspend fun setTermuxPreApprovedPatterns(patterns: String) =
        _settings.update { it.copy(termuxPreApprovedPatterns = patterns) }

    override suspend fun setMcpEnabled(enabled: Boolean) =
        _settings.update { it.copy(mcpEnabled = enabled) }

    override suspend fun setMcpDeferredLoading(enabled: Boolean) =
        _settings.update { it.copy(mcpDeferredLoading = enabled) }

    override suspend fun setMcpServersJson(json: String) =
        _settings.update { it.copy(mcpServersJson = json) }
}
