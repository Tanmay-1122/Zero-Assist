...
class ConfigTomlBuilderTest {
    private fun sectionBody(toml: String, section: String): String =
        toml
    ...
    inner class BuildGlobalConfig {
        ...
        @Test
        @DisplayName("fallback providers are ignored")
        fun `fallback providers are ignored`() {
            val toml =
                ConfigTomlBuilder.build(
                    GlobalTomlConfig(
                        provider = "",
        ...
        @Test
        @DisplayName("default reliability values omit reliability section")
        ...
        @Test
        @DisplayName("web fetch disabled emits disabled web_fetch section")
        fun `web fetch disabled emits disabled web_fetch section`() {
            val toml =
                ConfigTomlBuilder.build(
                    GlobalTomlConfig(
                        provider = "",
            ...
            val section = sectionBody(toml, "web_fetch")
            assertTrue(toml.contains("[web_fetch]"))
            assertTrue(section.contains("enabled = false"))
            assertFalse(section.contains("allowed_domains"))
        }
        ...
        @Test
        @DisplayName("http request disabled emits disabled http_request section")
        fun `http request disabled emits disabled http_request section`() {
            val toml =
                ConfigTomlBuilder.build(
                    GlobalTomlConfig(
                        provider = "",
            ...
            val section = sectionBody(toml, "http_request")
            assertTrue(toml.contains("[http_request]"))
            assertTrue(section.contains("enabled = false"))
            assertFalse(section.contains("allowed_domains"))
        }
        ...
        @Test
        @DisplayName("http request enabled without allowlist emits disabled section")
        fun `http request enabled without allowlist emits disabled section`() {
            val toml =
                ConfigTomlBuilder.build(
                    GlobalTomlConfig(
                        provider = "",
                        ...
                        baseUrl = "",
                        httpRequestEnabled = true,
                        httpRequestAllowedDomains = emptyList(),
                    ),
                )
            val section = sectionBody(toml, "http_request")
            assertTrue(toml.contains("[http_request]"))
        ...
        @Test
        @DisplayName("browser disabled emits disabled browser section")
        fun `browser disabled emits disabled browser section`() {
            val toml =
                ConfigTomlBuilder.build(
                    GlobalTomlConfig(
                        provider = "",
            ...
            val section = sectionBody(toml, "browser")
            assertTrue(toml.contains("[browser]"))
            assertTrue(section.contains("enabled = false"))
        }

        @Test
        @DisplayName("workflow folder enabled emits workflow folder section")
        fun `workflow folder enabled emits workflow folder section`() {
            val toml =
                ConfigTomlBuilder.build(
                    GlobalTomlConfig(
                        provider = "",
            ...
            val section = sectionBody(toml, "workflow_folder")
            assertTrue(section.contains("enabled = true"))
        }

        @Test
        @DisplayName("workflow folder disabled omits workflow folder section")
        fun `workflow folder disabled omits workflow folder section`() {
            val toml =
                ConfigTomlBuilder.build(
                    GlobalTomlConfig(
                        provider = "",
                        ...
                        apiKey = "",
                        baseUrl = "",
                        workflowFolderEnabled = false,
                    ),
                )

            assertFalse(toml.contains("[workflow_folder]"))
        }

        @Test
        @DisplayName("shared folder requires a selected URI")
        fun `shared folder requires selected uri`() {
            val disabledToml =
                ConfigTomlBuilder.build(
                    GlobalTomlConfig(
                        provider = "",
            ...
            val enabledToml =
                ConfigTomlBuilder.build(
                    GlobalTomlConfig(
                        ...
                        baseUrl = "",
                        sharedFolderEnabled = true,
                        sharedFolderUri = "content://tree/shared",
                    ),
                )

            assertFalse(disabledToml.contains("[shared_folder]"))
            assertTrue(sectionBody(enabledToml, "shared_folder").contains("enabled = true"))
        }

        @Test
        @DisplayName("web search disabled emits disabled web_search section")
        fun `web search disabled emits disabled web_search section`() {
            val toml =
                ConfigTomlBuilder.build(
                    GlobalTomlConfig(
                        provider = "",
            ...
            val section = sectionBody(toml, "web_search")
            assertTrue(toml.contains("[web_search]"))
            assertTrue(section.contains("enabled = false"))
        }

        @Test
        @DisplayName("Composio sessions key remains in composio section")
        fun `composio sessions key remains in composio section`() {
            val toml =
                ConfigTomlBuilder.build(
                    GlobalTomlConfig(
                        provider = "",
                        ...
                        composioEntityId = "youtube",
                    ),
                )

            assertTrue(toml.contains("[composio]"))
            assertTrue(toml.contains("enabled = true"))
            assertTrue(toml.contains("""api_key = "ck_test_sessions_key""""))
            assertTrue(toml.contains("""entity_id = "youtube""""))
            assertFalse(toml.contains("[mcp]"))
            assertFalse(toml.contains("x-consumer-api-key"))
        }

        @Test
        @DisplayName("Composio blank key emits inactive section")
        fun `composio blank key emits inactive section`() {
            val toml =
                ConfigTomlBuilder.build(
                    GlobalTomlConfig(
                        provider = "",
            ...
            val section = sectionBody(toml, "composio")

            assertTrue(section.contains("enabled = false"))
            assertFalse(section.contains("api_key"))
            assertFalse(section.contains("entity_id"))
        }

        @Test
        @DisplayName("Composio CLI user key emits inactive section")
        fun `composio cli user key emits inactive section`() {
            val toml =
                ConfigTomlBuilder.build(
                    GlobalTomlConfig(
                        provider = "",
                        ...
                        baseUrl = "",
                        composioEnabled = true,
                        composioApiKey = "uak_test_user_key",
                        composioEntityId = "default",
                    ),
                )
            val section = sectionBody(toml, "composio")
        ...
        @Test
        @DisplayName("webauthn enabled emits security.webauthn section")
        fun `webauthn enabled emits security webauthn section`() {
            val toml =
                ConfigTomlBuilder.build(
                    GlobalTomlConfig(
                        provider = "",
            ...
            assertTrue(toml.contains("""rp_origin = "https://agent.local""""))
            assertTrue(toml.contains("""rp_name = "Zero Assist""""))
        }

        @Test
        @DisplayName("default webauthn config omits security.webauthn section")
        fun `default webauthn config omits section`() {
            val toml =
                ConfigTomlBuilder.build(
                    GlobalTomlConfig(
                        provider = "",
        ...
        fun `autonomy section emits non cli excluded tools`() {
            ...
            assertTrue(toml.contains("""non_cli_excluded_tools = []"""))
        }
    }
    ...
    inner class ResolveProvider {
        ...
        @Test
        @DisplayName("ollama with custom URL stays ollama")
        fun `ollama with custom URL stays ollama`() {
            assertEquals(
                "ollama",
                ConfigTomlBuilder.resolveProvider("ollama", "http://10.0.2.2:11434"),
            )
        }
    ...
    inner class BuildChannelsToml {
        ...
        @Test
        @DisplayName("MQTT channel emits enabled SOP listener TOML")
        fun `mqtt channel emits enabled SOP listener TOML`() {
            val channel = ConnectedChannel(id = "7", type = ChannelType.MQTT)
            val values =
                mapOf(
                    "broker_url" to "mqtt://broker.local:1883",
                    "client_id" to "zero-assist-test",
                    "topics" to "sensors/#, alerts/+/critical",
                    "qos" to "1",
                    "use_tls" to "false",
            ...
            assertTrue(toml.contains("[channels_config.mqtt]"))
            assertTrue(toml.contains("enabled = true"))
            assertTrue(toml.contains("""broker_url = "mqtt://broker.local:1883""""))
            assertTrue(toml.contains("""client_id = "zero-assist-test""""))
            assertTrue(toml.contains(""""sensors/#""""))
            assertTrue(toml.contains(""""alerts/+/critical""""))
            assertTrue(toml.contains("qos = 1"))
            assertTrue(toml.contains("use_tls = false"))
            assertTrue(toml.contains("keep_alive_secs = 30"))
        }
