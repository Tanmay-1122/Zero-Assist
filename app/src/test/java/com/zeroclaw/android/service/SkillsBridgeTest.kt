...
class SkillsBridgeTest {
    ...
    @Test
    @DisplayName("generateAndInstallSkill delegates to FFI")
    fun `generateAndInstallSkill delegates to FFI`() =
        runTest {
            every {
                com.zeroclaw.ffi.generateAndInstallSkill("Summarize PDFs", "tester")
            } returns "summarize-pdfs"

            val result = bridge.generateAndInstallSkill("Summarize PDFs", "tester")

            assertEquals("summarize-pdfs", result)
            verify(exactly = 1) {
                com.zeroclaw.ffi.generateAndInstallSkill("Summarize PDFs", "tester")
            }
        }
    ...
    @Test
    @DisplayName("generateAndInstallSkill propagates FfiException")
    fun `generateAndInstallSkill propagates FfiException`() =
        runTest {
            every {
                com.zeroclaw.ffi.generateAndInstallSkill(any(), any())
            } throws FfiException.SpawnException("generate failed")

            assertThrows<FfiException> {
                bridge.generateAndInstallSkill("bad skill", null)
            }
        }
