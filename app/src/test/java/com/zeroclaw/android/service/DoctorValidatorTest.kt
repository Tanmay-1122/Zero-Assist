...
import com.zeroclaw.android.service.termux.TermuxBridgeHealthDetails
...
class DoctorValidatorTest {
    ...
    private var configValidationResult = ""
    ...
    fun setUp() {
        ...
        validator =
            DoctorValidator(
                ...
                configValidationBridge = ConfigValidationBridge { configValidationResult },
            )
    }
    ...
    inner class ConfigChecks {
        ...
        fun `invalid config fails`() =
            runTest {
                ...
                configValidationResult = "expected value, found EOF"
    ...
    @Nested
    @DisplayName("Termux checks")
    inner class TermuxChecks {
        @Test
        @DisplayName("passes package permission and bridge checks when Termux is ready")
        fun `ready termux checks pass`() =
            runTest {
                val validator =
                    DoctorValidator(
                        context = context,
                ...
                val checks = validator.runTermuxChecks()

                assertEquals(CheckStatus.PASS, checks.first { it.id == "termux-package" }.status)
                assertEquals(CheckStatus.PASS, checks.first { it.id == "termux-run-command-permission" }.status)
                assertEquals(CheckStatus.PASS, checks.first { it.id == "termux-bridge-health" }.status)
                val proot = checks.first { it.id == "termux-proot" }
                assertEquals(CheckStatus.PASS, proot.status)
                assertTrue(proot.detail.contains("debian"))
            }

        @Test
        @DisplayName("fails bridge readiness when Termux is not installed")
        fun `missing termux fails readiness checks`() =
            runTest {
                val validator =
                    DoctorValidator(
                        context = context,
                        ...
                        termuxRuntimeStatusProvider =
                            StaticTermuxStatusProvider(
                                readyTermuxStatus(
                                    ...
                                    bootstrapAvailability = TermuxBootstrapAvailability.UNAVAILABLE,
                                    health =
                                        TermuxHealthSnapshot(
                ...
                assertEquals(CheckStatus.FAIL, checks.first { it.id == "termux-package" }.status)
                assertEquals(CheckStatus.FAIL, checks.first { it.id == "termux-run-command-permission" }.status)
                assertEquals(CheckStatus.FAIL, checks.first { it.id == "termux-bridge-health" }.status)
            }
    }
    ...
    private class StaticTermuxStatusProvider(
        private val status: TermuxRuntimeStatus,
    ) : TermuxRuntimeStatusProvider {
        override suspend fun currentStatus(): TermuxRuntimeStatus = status
    }
