...
class ScreenObserverTest {
    ...
    @Test
    @DisplayName("trees within maxNodes skip the ranking sort entirely")
    fun `small trees skip sort`() {
        // Input order deliberately differs from ranking order: a non-actionable,
        // blank-label node first and a clickable node second. If the comparator
        // ran, the clickable node would be re-indexed [0]; because the fast
        // path skips sorting when size <= maxNodes, input order is preserved.
        every { mockService.snapshot() } returns listOf(
            UiNodeSnapshot(
                index = 0, text = "", contentDescription = "",
                className = "TextView", viewId = "", clickable = false,
                editable = false, scrollable = false, checkable = false,
                checked = false, focused = false, focusable = false,
                enabled = true, bounds = Rect(0, 0, 100, 100), depth = 0,
        ...
        val observer = ScreenObserver(maxNodes = 2)
        val (desc, _, _) = observer.describeWithFingerprint(mockService, "test")

        val blankLinePos = desc.indexOf("[0] TextView")
        val buttonPos = desc.indexOf("[1] Button")
        assertTrue(blankLinePos != -1 && buttonPos != -1)
        assertTrue(
            blankLinePos < buttonPos,
            "With size == maxNodes the input order must be preserved (sort skipped)",
        )
    }
}
