...
export function AgentProvider({ agentAlias, children }: AgentProviderProps) {
  ...
  const toggleCanvasPanel = useCallback(() => setCanvasPanelOpen((v) => !v), []);
      ...
      case 'tool_result': {
        ...
        setMessages((prev) => {
          ...
          if (idx !== -1) {
            ...
            const toolCall = { ...existing.toolCall!, output: msg.output ?? '' };
            if (msg.canvasFrame) {
              toolCall.canvasFrame = {
                canvasId: msg.canvasFrame.canvas_id,
                frameId: msg.canvasFrame.frame_id,
                contentType: msg.canvasFrame.content_type,
                content: msg.canvasFrame.content,
              };
            }
            updated[idx] = { ...existing, toolCall };
            return updated;
          }
          const fallbackToolCall: ToolCallInfo = { name: toolName, output: msg.output ?? '' };
          if (msg.canvasFrame) {
            fallbackToolCall.canvasFrame = {
              canvasId: msg.canvasFrame.canvas_id,
              frameId: msg.canvasFrame.frame_id,
              contentType: msg.canvasFrame.content_type,
              content: msg.canvasFrame.content,
