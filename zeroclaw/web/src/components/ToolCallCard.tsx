...
import {
  ...
  Image, Camera, Calculator, Wrench, CheckCircle2, Loader2, Monitor,
} from 'lucide-react';
import { Card, Badge } from '@/components/ui';
import { t } from '@/lib/i18n';
import CanvasInlineBlock from './CanvasInlineBlock';
...
export default function ToolCallCard({ toolCall }: ToolCallCardProps) {
  ...
  return (
    <Card padded={false} className="bg-pc-elevated p-3 text-xs">
      ...
      {toolCall.name === 'canvas' && toolCall.canvasFrame ? (
        <div className="mt-2">
          <CanvasInlineBlock
            canvasId={toolCall.canvasFrame.canvasId}
            contentType={toolCall.canvasFrame.contentType}
            content={toolCall.canvasFrame.content}
          />
        </div>
      ) : (
        <>
