...
function WebToggleCard() {
  const [enabled, setEnabled] = useState<boolean | null>(null);
  const [toggling, setToggling] = useState(false);

  useEffect(() => {
    let cancelled = false;
    fetch("/admin/web/status")
      .then((r) => r.json())
      .then((d: { web_enabled: boolean }) => {
        if (!cancelled) setEnabled(d.web_enabled);
      })
      .catch(() => {
  ...
  const toggle = useCallback(async () => {
    ...
    setToggling(true);
    try {
      const r = await fetch("/admin/web/toggle", { method: "POST" });
      const d = (await r.json()) as { web_enabled: boolean };
      setEnabled(d.web_enabled);
    } catch {
      /* ignore */
    } finally {
      setToggling(false);
  ...
  return (
    <div
      className="card p-5 animate-slide-in-up cursor-pointer select-none"
      onClick={toggle}
      role="button"
      tabIndex={0}
    ...
    >
      ...
      <p className="text-sm truncate" style={{ color: "var(--pc-text-muted)" }}>
        {isOn ? t("dashboard.web_toggle_on") : t("dashboard.web_toggle_off")}
      </p>
    </div>
  );
}
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 stagger-children">
      {channels.map((channel) => (
        <div
          key={channel.name}
          className="card p-[var(--layout-card-padding)] animate-slide-in-up transition-all"
          style={{
...
export default function Dashboard() {
  ...
  return (
    <div className="px-[var(--layout-page-padding)] pt-6 pb-8 space-y-[var(--layout-section-gap)] animate-fade-in max-w-[var(--layout-content-max-width)] mx-auto">
      <AgentsSection />
...
function HealthTab({ status }: { status: StatusResponse }) {
  ...
  if (entries.length === 0) {
    return (
      <div className="card p-[var(--layout-card-padding)] animate-slide-in-up">
        <p className="text-sm" style={{ color: "var(--pc-text-faint)" }}>
          {t("dashboard.no_components")}
        </p>
      </div>
...
function CostTab({
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-[var(--layout-section-gap)]">
        <div className="card p-[var(--layout-card-padding)] animate-slide-in-up">
          <div className="flex items-center gap-2 mb-5">
        ...
        <div className="card p-[var(--layout-card-padding)] lg:col-span-2 animate-slide-in-up">
          <div className="flex items-center gap-2 mb-5">
            <DollarSign
              className="h-5 w-5"
              style={{ color: "var(--pc-accent)" }}
...
function MemoriesTab() {
  ...
  return (
    <div className="card p-[var(--layout-card-padding)] animate-slide-in-up space-y-4">
      <div className="flex items-center gap-2 flex-wrap">
...
function AgentsSection() {
  ...
  return (
    <section className="space-y-[var(--layout-section-gap)]">
      <PageHeader
        title={t("dash.title")}
