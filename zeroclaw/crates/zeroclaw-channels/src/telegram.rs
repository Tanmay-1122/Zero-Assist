            ...
            if let Some(results) = data.get("result").and_then(serde_json::Value::as_array) {
                for update in results {
                    ...
                    if let Some(cb) = update.get("callback_query") {
                        ...
                        // Advance offset for callback queries — they are
                        // fully processed inline and don't go through tx.send().
                    ...
                    // Advance offset AFTER the message has been dispatched to
                    // the orchestrator. This prevents losing updates if the
                    // process crashes mid-batch — un-sent updates will be
