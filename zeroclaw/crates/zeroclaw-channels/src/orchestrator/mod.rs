...
pub use crate::irc::{IrcChannel, IrcChannelConfig};
...
const MEMORY_CONTEXT_MAX_ENTRIES: usize = 3;
...
struct ChannelRuntimeContext {
    ...
    channel_trust_levels: Arc<std::collections::HashMap<String, zeroclaw_config::schema::ChannelTrustLevel>>,
    default_channel_trust_level: zeroclaw_config::schema::ChannelTrustLevel,
...
async fn build_memory_context(
    ...
    if let Ok(entries) = mem.recall(user_msg, 3, session_id, None, None).await {
        ...
        for entry in entries.iter().filter(|e| match e.score {
            Some(score) => score >= min_relevance_score && score >= 0.5,
            ...
            // Relevance gap filter: skip if score drops > 50% from last included
...
fn parse_reply_intent(response: &str) -> AssistantChannelOutcome {
    ...
    // ponytail: empty/garbled classifier output defaults to REPLY — better to
    // answer a question twice than silently drop it.
    if trimmed.is_empty() {
...
/// Retry a channel send with exponential backoff for transient errors
/// (rate limits, network timeouts). Returns Ok(()) on success or the last
    ...
    loop {
        match channel.send(msg).await {
            ...
            Err(e) => {
                ...
                if is_transient && (attempt as usize) <= max_retries {
                    ...
                    tracing::warn!(
                        ...
                        error = %e,
                        "Transient send error; retrying in {backoff:?}"
                    );
...
async fn process_channel_message(
    ...
    ctx.observer.record_event(&ObserverEvent::ChannelMessage {
        channel: msg.channel.clone(),
        direction: "inbound".into(),
        sender: Some(msg.sender.clone()),
        content_preview: Some(truncate_with_ellipsis(&msg.content, 160)),
    });
    ...
    tracing::info!(
        ...
        sender = %msg.sender,
        always_reply,
        message_preview = %msg.content.chars().take(80).collect::<String>(),
        "Reply-intent precheck starting"
    );

    let reply_intent = if always_reply {
        tracing::info!(
            channel = %msg.channel,
            sender = %msg.sender,
            "Reply-intent bypassed via always_reply"
        );
        AssistantChannelOutcome::Reply(String::new())
    } else {
        match classify_channel_reply_intent(
        ...
        {
            Ok(outcome) => {
                let (kind_str, reason_str) = match &outcome {
                    AssistantChannelOutcome::Reply(_) => ("REPLY".to_string(), String::new()),
                    AssistantChannelOutcome::NoReply { kind, reason } => (
                        format!("{kind:?}"),
                        reason.as_deref().unwrap_or("none").to_string(),
                    ),
                };
                tracing::info!(
                    channel = %msg.channel,
                    sender = %msg.sender,
                    classifier_result = %kind_str,
                    reason = %reason_str,
                    "Reply-intent classifier decision"
                );
                outcome
            }
            Err(e) => {
                tracing::warn!(
                    ...
                    sender = %msg.sender,
                    error = %e,
                    "Reply-intent classifier failed, defaulting to REPLY"
                );
                AssistantChannelOutcome::Reply(String::new())
            }
        }
    ...
    if let AssistantChannelOutcome::NoReply { kind, reason } = reply_intent {
        ...
        // Send a short text acknowledgment so the user isn't left
        // staring at an emoji with no explanation.
        if let Some(channel) = target_channel.as_ref() {
            let ack_text = match kind {
                NoReplyKind::Informational => "👍 Noted — no reply needed.".to_string(),
                NoReplyKind::Refused => "🚫 Can't do that — check the reason in logs.".to_string(),
                NoReplyKind::Failed => format!(
                    "⚠️ Tried but couldn't fulfil: {}",
                    reason.as_deref().unwrap_or("unknown error")
                ),
            };
            if let Err(e) = channel
                .send(
                    &SendMessage::new(&ack_text, &msg.reply_target)
                        .in_thread(msg.thread_ts.clone()),
                )
                .await
            {
                tracing::debug!(
                    "Failed to send no-reply acknowledgment on {}: {e}",
                    channel.name()
                );
            }
        }
    ...
    // Compute effective excluded tools for this message based on trust level
    let trust_excluded: Vec<String> = {
        let channel_trust = if msg.channel == "cli" {
            zeroclaw_config::schema::ChannelTrustLevel::Full
        } else {
            ctx.channel_trust_levels
                .get(&msg.channel)
                .copied()
                .unwrap_or(ctx.default_channel_trust_level)
        };
        match channel_trust {
            zeroclaw_config::schema::ChannelTrustLevel::Full => Vec::new(),
            zeroclaw_config::schema::ChannelTrustLevel::Standard => {
                vec!["shell".to_string(), "file_write".to_string()]
            }
            zeroclaw_config::schema::ChannelTrustLevel::Restricted => {
                vec![
                    "shell".to_string(), "file_write".to_string(),
                    "file_edit".to_string(), "shell_safe_exec".to_string(),
                    "browser_open".to_string(), "composio".to_string(),
                    "delegate".to_string(), "schedule".to_string(),
                    "memory_store".to_string(), "memory_forget".to_string(),
                ]
            }
        }
    };
    let effective_excluded: Vec<String> = if msg.channel == "cli" || ctx.autonomy_level == AutonomyLevel::Full {
        Vec::new()
    } else {
        let mut merged = ctx.non_cli_excluded_tools.as_ref().clone();
        for tool in &trust_excluded {
    ...
    match llm_result {
        ...
        LlmExecutionResult::Completed(Ok(Ok(response))) => {
            ...
            if let Some(channel) = target_channel.as_ref() {
                ...
                } else if let Err(e) = send_with_retry(
                    channel.as_ref(),
                    &SendMessage::new(&delivered_response, &msg.reply_target)
                        .in_thread(msg.thread_ts.clone())
                ...
                .await
                {
                    tracing::error!(
                        ...
                        message_id = %msg.id,
                        error = %e,
                        "Failed to send reply after retries"
                    );
                    eprintln!("  ❌ Failed to reply on {}: {e}", channel.name());
                    // Best-effort: tell the user something went wrong so they
                    // aren't left wondering why the bot went silent.
                    let _ = channel
                        .send(
                            &SendMessage::new(
                                "⚠️ Sorry — failed to deliver the full reply. Please try again.",
                                &msg.reply_target,
                            )
                            .in_thread(msg.thread_ts.clone()),
                        )
...
fn collect_configured_channels(
    ...
    // MQTT bidirectional AI chat channel
    if let Some(ref mqtt) = config.channels.mqtt {
        if mqtt.enabled && mqtt.chat_enabled {
            channels.push(ConfiguredChannel {
                display_name: "MQTT",
                channel: Arc::new(mqtt_chat::MqttChatChannel::new(mqtt.clone())),
            });
        } else if mqtt.enabled && !mqtt.chat_enabled {
            tracing::info!("MQTT channel enabled but chat_enabled is false (SOP-only mode)");
        }
    }
...
pub async fn start_channels(config: Config) -> Result<()> {
    start_channels_with_observer(config, None).await
}

/// Start all configured channels with an optional additional observer.
///
/// When `additional_observer` is provided, it is wrapped together with the
/// config-based observer in a [`MultiObserver`] so that events fan out to
/// both backends. This allows the Android FFI layer to inject the
/// [`AndroidObserver`] which forwards events to Kotlin's `EventBridge`.
#[allow(clippy::too_many_lines)]
pub async fn start_channels_with_observer(
    config: Config,
    additional_observer: Option<Box<dyn zeroclaw_runtime::observability::traits::Observer>>,
) -> Result<()> {
    ...
    let observer: Arc<dyn Observer> = match additional_observer {
        Some(extra) => {
            let base: Box<dyn Observer> = observability::create_observer(&config.observability);
            Arc::new(observability::MultiObserver::new(vec![base, extra]))
        }
        None => Arc::from(observability::create_observer(&config.observability)),
    };
    ...
    if config.mcp.enabled && !active_mcp_servers.is_empty() {
        tracing::info!(
            "Initializing MCP client — {} of {} server(s) active",
    ...
    // Build tool descriptions for the system prompt dynamically from the
    // runtime tool registry so the prompt always matches available tools
    // (including termux, sandbox, etc.).
    // Collect as owned strings before built_tools is moved into Arc.
    let mut tool_descs_owned: Vec<(String, String)> = built_tools
        .iter()
        .map(|t| (t.name().to_string(), t.description().to_string()))
        .collect();

    // Ensure core tools are always present even if not in the registry
    let ensure_tools: &[(&str, &str)] = &[
    ...
    for (name, desc) in ensure_tools {
        if !tool_descs_owned.iter().any(|(n, _)| n == name) {
            tool_descs_owned.push((name.to_string(), desc.to_string()));
        }
    }
    ...
    tracing::info!(
        ...
        tool_names = ?tools_registry.iter().map(|t| t.name()).collect::<Vec<_>>(),
        device_control_present = tools_registry.iter().any(|t| t.name() == "device_control"),
        "Tool registry frozen for channel orchestrator"
    );

    // Convert to borrowed form after built_tools is moved
    let mut tool_descs: Vec<(&str, &str)> = tool_descs_owned
        .iter()
    ...
    // Trust-level based tool filtering at startup. Uses the default trust level
    // since we don't know the specific channel yet. Per-message trust filtering
    // is applied in the message handler (run_agent_for_message).
    let default_trust = config.autonomy.default_channel_trust_level;
    match default_trust {
        zeroclaw_config::schema::ChannelTrustLevel::Full => {}
        zeroclaw_config::schema::ChannelTrustLevel::Standard => {
            let restricted_tools = ["shell", "file_write"];
            tool_descs.retain(|(name, _)| !restricted_tools.contains(name));
        }
        zeroclaw_config::schema::ChannelTrustLevel::Restricted => {
            let allowed_tools = [
                "file_read", "memory_recall", "memory_store", "memory_forget",
                "web_search_tool", "web_fetch", "glob_search", "content_search",
                "calculator", "weather", "image_info", "read_skill",
            ];
            tool_descs.retain(|(name, _)| allowed_tools.contains(name));
        }
    }
    ...
    let runtime_ctx = Arc::new(ChannelRuntimeContext {
        ...
        channel_trust_levels: Arc::new(config.autonomy.channel_trust_levels.clone()),
        default_channel_trust_level: config.autonomy.default_channel_trust_level,
