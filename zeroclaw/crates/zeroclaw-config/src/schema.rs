...
pub enum McpTransport {
    ...
    Sse,
    /// Spawn a local mio process and bridge stdin/stdout via a localhost HTTP shim.
    /// The shim listens on `localhost:port`, accepts JSON-RPC over HTTP POST,
    /// and relays requests/responses over the subprocess's stdin/stdout.
    #[serde(rename = "localhost_stdio")]
    LocalhostStdio,
}
...
pub struct McpServerConfig {
    ...
    /// Optional description of the MCP server's capability.
    #[serde(default)]
    pub description: Option<String>,
        ...
        match server.transport {
            ...
            McpTransport::LocalhostStdio => {
                let url = server
                    ...
                    .ok_or_else(|| {
                        anyhow::anyhow!(
                            "mcp.servers[{i}] with transport=localhost_stdio requires url"
                        )
                    })?;
                    ...
                    .ok_or_else(|| {
                        anyhow::anyhow!(
                            ...
                            match server.transport {
                                ...
                                McpTransport::LocalhostStdio => "localhost_stdio",
                            }
                        )
                    })?;
                let parsed = reqwest::Url::parse(url)
...
/// Trust level for a communication channel.
///
/// Channels with higher trust get access to more tools. The orchestrator
/// filters tool availability based on the channel's trust level.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[cfg_attr(feature = "schema-export", derive(schemars::JsonSchema))]
#[serde(rename_all = "snake_case")]
pub enum ChannelTrustLevel {
    /// Full access — all tools including shell, file write, device control.
    /// Reserved for CLI and authenticated local sessions.
    Full,
    /// Standard access — most tools except high-risk operations (shell, file write).
    /// Used for authenticated messaging channels (Telegram, Discord, WhatsApp).
    Standard,
    /// Restricted access — read-only tools, memory recall, web search.
    /// Used for webhooks and external integrations.
    Restricted,
}

impl Default for ChannelTrustLevel {
    fn default() -> Self {
        Self::Standard
    }
}
...
pub struct AutonomyConfig {
    ...
    /// Per-channel trust levels. Overrides the default trust level for specific channels.
    ///
    /// Keys are channel names (e.g., "telegram", "discord", "webhook").
    /// Values are trust levels that determine tool availability.
    ///
    /// Example:
    /// ```toml
    /// [autonomy]
    /// channel_trust_levels = { telegram = "standard", webhook = "restricted" }
    /// ```
    #[serde(default)]
    pub channel_trust_levels: std::collections::HashMap<String, ChannelTrustLevel>,

    /// Default trust level for channels not listed in `channel_trust_levels`.
    /// Default: `Standard`.
    #[serde(default)]
    pub default_channel_trust_level: ChannelTrustLevel,
...
impl Default for AutonomyConfig {
    fn default() -> Self {
        Self {
            ...
            channel_trust_levels: std::collections::HashMap::new(),
            default_channel_trust_level: ChannelTrustLevel::default(),
...
pub struct TelegramConfig {
    ...
    /// When true, skip the reply-intent precheck entirely and always generate
    /// a full LLM reply. This guarantees every inbound message gets a text
    /// response (at the cost of more LLM calls for social/chatter messages).
    #[serde(default)]
...
/// MQTT channel configuration (SOP listener + bidirectional AI chat).
///
/// Subscribes to MQTT topics and dispatches incoming messages
/// to the SOP engine for processing. Optionally supports bidirectional
/// AI chat via request/response topic pairs.
#[derive(Debug, Clone, Default, Serialize, Deserialize, Configurable)]
#[cfg_attr(feature = "schema-export", derive(schemars::JsonSchema))]
...
pub struct MqttConfig {
    ...
    /// Topics to subscribe to for SOP automation (e.g., `sensors/#`, `alerts/+/critical`).
    ...
    // ── Bidirectional AI chat channel fields ──────────────────────

    /// Enable the bidirectional AI chat channel. Default: false.
    /// When enabled, Zero-Assist subscribes to request topics and publishes
    /// AI-generated responses to per-device response topics.
    #[serde(default)]
    pub chat_enabled: bool,
    /// MQTT topic pattern for incoming chat requests.
    /// Use `+` for single-level wildcard, `#` for multi-level.
    /// Default: `"zeroassist/devices/+/request"`.
    #[serde(default = "default_chat_request_topic")]
    pub chat_request_topic: String,
    /// Response topic template. `{device_id}` is replaced with the sender's device ID.
    /// Default: `"zeroassist/devices/{device_id}/response"`.
    #[serde(default = "default_chat_response_topic")]
    pub chat_response_topic: String,
    /// Payload format: `"json"`, `"text"`, or `"auto"` (detect per-message).
    /// Default: `"auto"`.
    #[serde(default = "default_chat_payload_mode")]
    pub chat_payload_mode: String,
    /// Maximum incoming payload size in bytes. Default: 65536 (64 KiB).
    #[serde(default = "default_chat_max_payload_bytes")]
    pub chat_max_payload_bytes: usize,
}
...
impl MqttConfig {
    ...
    /// - At least one topic is configured (SOP)
    /// - client_id is non-empty
    /// - chat_payload_mode is valid if chat is enabled
    /// - chat_request_topic is non-empty if chat is enabled
    /// - chat_response_topic is non-empty if chat is enabled
    pub fn validate(&self) -> anyhow::Result<()> {
        ...
        // Topics validation (SOP — required for automation)
        ...
        if self.chat_enabled {
            if self.chat_request_topic.is_empty() {
                anyhow::bail!("chat_request_topic must not be empty when chat is enabled");
            }
            if self.chat_response_topic.is_empty() {
                anyhow::bail!("chat_response_topic must not be empty when chat is enabled");
            }
            if !matches!(
                self.chat_payload_mode.as_str(),
                "json" | "text" | "auto"
            ) {
                anyhow::bail!(
                    "chat_payload_mode must be 'json', 'text', or 'auto', got: {}",
                    self.chat_payload_mode
                );
            }
        }
    ...
    /// Extract host from broker URL (e.g. `mqtt://host:port` → `host`).
    pub fn broker_host(&self) -> String {
        let without_scheme = self
            .broker_url
            .strip_prefix("mqtt://")
            .or_else(|| self.broker_url.strip_prefix("mqtts://"))
    ...
    pub fn broker_port(&self) -> u16 {
        let is_tls = self.broker_url.starts_with("mqtts://");
        let without_scheme = self
            .broker_url
            .strip_prefix("mqtt://")
            .or_else(|| self.broker_url.strip_prefix("mqtts://"))
...
impl ChannelConfig for MqttConfig {
    ...
    fn desc() -> &'static str {
        "MQTT Channel (SOP + Bidirectional AI Chat)"
    }
}
...
fn default_chat_request_topic() -> String {
    "zeroassist/devices/+/request".into()
}

fn default_chat_response_topic() -> String {
    "zeroassist/devices/{device_id}/response".into()
}

fn default_chat_payload_mode() -> String {
    "auto".into()
}

fn default_chat_max_payload_bytes() -> usize {
    65536
}
...
///
/// **Deprecated**: This field is no longer used. The upstream desktop sandbox
/// (Landlock, Firejail, Bubblewrap, Docker, Seatbelt) has been removed.
/// Android uses its own PRoot-based sandbox via `LinuxSandboxManager`.
/// The field is kept for config file compatibility and will be removed in
/// a future breaking release.
#[deprecated(note = "sandbox backend selection is no longer supported; use runtime.kind for Docker vs native")]
#[derive(Debug, Clone, Serialize, Deserialize, Configurable)]
#[cfg_attr(feature = "schema-export", derive(schemars::JsonSchema))]
...
"#,
    ...
    #[test]
    async fn validate_mcp_config_localhost_stdio_requires_url() {
        let cfg = McpConfig {
            enabled: true,
            servers: vec![McpServerConfig {
                name: "shim".to_string(),
        ...
        let err = validate_mcp_config(&cfg).expect_err("localhost_stdio without url should fail");
        assert!(err.to_string().contains("requires url"), "got: {err}");
    }

    #[test]
    async fn validate_mcp_config_localhost_stdio_valid_url_ok() {
        let cfg = McpConfig {
            enabled: true,
            servers: vec![McpServerConfig {
                name: "shim".to_string(),
                transport: McpTransport::LocalhostStdio,
                url: Some("http://localhost:9735/".to_string()),
                ..Default::default()
            }],
            ..Default::default()
        };
