...
/// Smart history truncation: three-tier strategy to preserve maximum context.
/// Returns the number of messages dropped.
///
/// Tier 1: Collapse assistant+tool pairs into compact summaries
/// Tier 2: Truncate large tool_result messages to save space
/// Tier 3: Drop oldest non-system messages (1/3 at a time, not 1/2)
fn truncate_for_context(messages: &mut Vec<ChatMessage>) -> usize {
    ...
    // Tier 2: Truncate large tool_result messages (head+tail with middle removed)
    const TOOL_TRIM_MAX: usize = 2000;
    for msg in messages.iter_mut() {
        if msg.role == "tool" && msg.content.len() > TOOL_TRIM_MAX {
            let head_len = TOOL_TRIM_MAX * 2 / 3;
            let tail_len = TOOL_TRIM_MAX.saturating_sub(head_len);
            let tail_start = msg.content.len().saturating_sub(tail_len);
            if head_len < tail_start {
                let truncated_chars = tail_start - head_len;
                msg.content = format!(
    ...
    // Recalculate remaining non-system count after collapse
    let remaining_non_system: Vec<usize> = messages
        .iter()
        .enumerate()
        .filter(|(_, m)| m.role != "system")
        .map(|(i, _)| i)
    ...
    if remaining_non_system.len() <= 1 {
        return dropped;
    }

    // Tier 3: Only if still too many, drop oldest 1/3 (not 1/2)
    let drop_count = remaining_non_system.len().saturating_sub(1) / 3;
    if drop_count > 0 {
        let indices_to_remove: Vec<usize> = remaining_non_system[..drop_count].to_vec();
