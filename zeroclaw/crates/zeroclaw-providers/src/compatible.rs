...
impl Provider for OpenAiCompatibleProvider {
    ...
    async fn chat_with_history(
            ...
            .await
        {
            ...
            Err(chat_error) => {
                tracing::error!(
                    ...
                    provider = self.name,
                    error = %chat_error,
                    "HTTP_TRANSPORT_ERROR chat_with_history"
                );
                if self.supports_responses_fallback {
                    let sanitized = super::sanitize_api_error(&chat_error.to_string());
                    return self
        ...
        if !response.status().is_success() {
            ...
            return Err(anyhow::anyhow!("{} API error ({status}): {sanitized}", self.name));
        }
    ...
    fn stream_chat(
        ...
        let stream_options = if options.enabled {
