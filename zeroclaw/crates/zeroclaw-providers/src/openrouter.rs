...
impl OpenRouterProvider {
    ...
    fn convert_messages(messages: &[ChatMessage]) -> Vec<NativeMessage> {
        ...
        for m in messages {
            ...
            {
                pending_tool_call_ids = parsed_calls.iter().map(|tc| tc.id.clone()).collect();
                ...
                && let Ok(value) = serde_json::from_str::<serde_json::Value>(&m.content)
            {
                ...
                let Some(pending_index) = pending_tool_call_ids
                    .iter()
                    .position(|id| id == &tool_call_id)
                else {
                    continue;
                };
                pending_tool_call_ids.remove(pending_index);
                let content = value
                    .get("content")
                    .and_then(serde_json::Value::as_str)
                    .map(|value| MessageContent::Text(value.to_string()))
    ...
    async fn chat(
        ...
        if !response.status().is_success() {
            ...
            tracing::error!(
                ...
                request_body = %body_json,
                "HTTP_ERR openrouter_chat"
            );
            let sanitized = super::sanitize_api_error(&response_body);
            return Err(anyhow::anyhow!("OpenRouter API error ({status}): {sanitized}"));
        }
...
mod tests {
    ...
    fn convert_messages_parses_tool_result_payload() {
        let messages = vec![
            ChatMessage {
                role: "assistant".into(),
                content: r#"{"content":"Using tool","tool_calls":[{"id":"call_xyz","name":"shell","arguments":"{\"command\":\"pwd\"}"}]}"#
                    .into(),
            },
            ChatMessage {
                role: "tool".into(),
        ...
        assert_eq!(converted[1].tool_call_id.as_deref(), Some("call_xyz"));
        assert_eq!(
            converted[1]
                .content
                .as_ref()
    ...
    #[test]
    fn convert_messages_drops_orphan_tool_result_payload() {
        let messages = vec![
            ChatMessage {
                role: "user".into(),
                content: "hello".into(),
            ...
            ChatMessage {
                role: "tool".into(),
                content: r#"{"tool_call_id":"call_orphan","content":"stale"}"#.into(),
            },
            ChatMessage {
                role: "assistant".into(),
                content: "Ready.".into(),
