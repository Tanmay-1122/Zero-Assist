...
impl GeminiProvider {
    async fn send_generate_content(
        ...
        tracing::info!(
            target: "zeroclaw::http_capture",
            provider = "Gemini",
            model = model,
            url = %url,
            request_body = %request_json,
            "HTTP_REQ gemini_send_generate_content"
        );
        ...
        if !status.is_success() {
            ...
            tracing::error!(
                ...
                request_body = %request_json,
                "HTTP_ERR gemini_send_generate_content"
            );
        ...
        if !response.status().is_success() {
            ...
            tracing::error!(
                ...
                response_body = %error_text,
                request_body = %request_json,
                "HTTP_ERR gemini_send_generate_content (after retries)"
            );
            anyhow::bail!("Gemini API error ({status}): {error_text}");
        }
        ...
        tracing::info!(
            ...
            status = %status,
            request_body = %request_json,
            "HTTP_OK gemini_send_generate_content"
        );
...
impl Provider for GeminiProvider {
    ...
    async fn chat_with_history(
        ...
        let temperature = temperature.unwrap_or(self.default_temperature());
        let (contents, system_instruction) = prepare_gemini_payload(messages)?;
...
fn prepare_gemini_payload(
    messages: &[ChatMessage],
) -> anyhow::Result<(Vec<Content>, Option<Content>)> {
    let mut system_parts: Vec<String> = Vec::new();

    // 1. Clean up and extract system / tool messages.
    ...
    for msg in messages {
        ...
        if msg.role == "tool" {
            // Drop native tool result messages.
            continue;
        }
        if msg.role == "assistant" {
            // If this is a JSON-formatted assistant message from native tool calling, extract the content text.
            if let Ok(value) = serde_json::from_str::<serde_json::Value>(&msg.content) {
                if value.get("tool_calls").is_some() {
                    let text = value
                        ...
                        .unwrap_or("")
                        .to_string();
                    cleaned.push(ChatMessage::assistant(text));
                    continue;
                }
            }
        }
    ...
    let mut coalesced: Vec<ChatMessage> = Vec::new();
    for msg in cleaned {
        match coalesced.last_mut() {
            Some(last) if last.role == msg.role => {
                let content_to_append = msg.content.clone();
                if !last.content.is_empty() && !content_to_append.is_empty() {
                    last.content.push_str("\n\n");
                }
                last.content.push_str(&content_to_append);
            }
    ...
    for msg in coalesced {
        ...
        let parts = if role == "user" {
            let p = build_parts(&msg.content);
            // Ensure no part is empty.
            p.into_iter()
                .map(|part| match part {
                    Part::Text { text } => {
                        let trimmed = text.trim().to_string();
                        Part::text(if trimmed.is_empty() { "..." } else { &trimmed })
                    }
                    other => other,
                })
                .collect()
            ...
            vec![Part::text(if trimmed.is_empty() {
    ...
    // 4. Ensure strict role alternation and boundary constraints.
    if contents.is_empty() {
        anyhow::bail!("Gemini requires at least one message in history");
    }

    // Ensure the first message is "user".
    if contents[0].role.as_deref() == Some("model") {
        contents.insert(
            0,
            Content {
                role: Some("user".to_string()),
    ...
    if contents.last().and_then(|c| c.role.as_deref()) == Some("model") {
        contents.push(Content {
            role: Some("user".to_string()),
...
mod tests {
    ...
    #[test]
    fn test_prepare_gemini_payload_handling() {
        // 1. Basic alternation and system extraction
        let messages = vec![
            ChatMessage::system("Sys instruction"),
            ChatMessage::user("Hi"),
            ChatMessage::assistant("Hello"),
            ChatMessage::user("How are you?"),
        ];
        let (contents, system) = prepare_gemini_payload(&messages).unwrap();

        // System instruction is extracted
        assert!(system.is_some());
        let sys_json = serde_json::to_value(&system.unwrap()).unwrap();
        assert_eq!(
            sys_json["parts"][0]["text"].as_str(),
            Some("Sys instruction")
        );
        ...
        // 2. Coalescing consecutive same roles
        let messages_dup = vec![
            ChatMessage::user("Part 1"),
            ChatMessage::user("Part 2"),
            ChatMessage::assistant("Ans 1"),
            ChatMessage::assistant("Ans 2"),
            ChatMessage::user("Final"),
        ];
        let (contents_dup, _) = prepare_gemini_payload(&messages_dup).unwrap();
        assert_eq!(contents_dup.len(), 3);

        let val_user_1 = serde_json::to_value(&contents_dup[0]).unwrap();
        assert_eq!(
            val_user_1["parts"][0]["text"].as_str(),
            Some("Part 1\n\nPart 2")
        );

        let val_model = serde_json::to_value(&contents_dup[1]).unwrap();
        assert_eq!(
            val_model["parts"][0]["text"].as_str(),
            Some("Ans 1\n\nAns 2")
        );

        // 3. Empty message body handling
        let messages_empty = vec![
            ChatMessage::user(""),
            ChatMessage::assistant("  "),
            ChatMessage::user("  "),
        ];
        let (contents_empty, _) = prepare_gemini_payload(&messages_empty).unwrap();
        assert_eq!(contents_empty.len(), 3);

        let user_empty_val = serde_json::to_value(&contents_empty[0]).unwrap();
        assert_eq!(user_empty_val["parts"][0]["text"].as_str(), Some("..."));

        let model_empty_val = serde_json::to_value(&contents_empty[1]).unwrap();
        assert_eq!(
            model_empty_val["parts"][0]["text"].as_str(),
            Some("[Calling tools...]")
        );

        // 4. Stripping native tool messages
        let native_tool_json = serde_json::json!({
        ...
        let messages_native = vec![
            ChatMessage::user("List files please"),
            ChatMessage::assistant(native_tool_json.to_string()),
            ChatMessage::tool("tool result"),
            ChatMessage::assistant("Here is the list"),
        ];

        let (contents_native, _) = prepare_gemini_payload(&messages_native).unwrap();
        // The tool message is dropped.
        // The first assistant is mapped to its "content".
        // The second assistant is appended, and they coalesce since the tool msg between them is gone.
        assert_eq!(contents_native.len(), 2);
        assert_eq!(contents_native[0].role.as_deref(), Some("user"));
        assert_eq!(contents_native[1].role.as_deref(), Some("model"));

        let native_model_val = serde_json::to_value(&contents_native[1]).unwrap();
        assert_eq!(
            native_model_val["parts"][0]["text"].as_str(),
            Some("Running shell command...\n\nHere is the list")
        );

        // 5. Boundary requirements
        let messages_model_boundary = vec![
            ChatMessage::assistant("I start"),
            ChatMessage::user("Middle"),
            ChatMessage::assistant("I end"),
        ];
        let (contents_boundary, _) = prepare_gemini_payload(&messages_model_boundary).unwrap();
        assert_eq!(contents_boundary.len(), 5);
        assert_eq!(contents_boundary[0].role.as_deref(), Some("user"));
        assert_eq!(contents_boundary[4].role.as_deref(), Some("user"));
    }
}
