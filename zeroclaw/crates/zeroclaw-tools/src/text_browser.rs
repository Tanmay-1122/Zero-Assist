...
impl TextBrowserTool {
    ...
    /// Set PRoot configuration for executing text browser commands inside PRoot.
    ...
    async fn detect_browser(&self) -> Option<String> {
        for browser in SUPPORTED_BROWSERS {
            let found = if let Some(ref proot) = self.proot_config {
                if proot.enabled {
                    // Check inside PRoot
                    proot.execute_command("which", &[browser])
                        .await
                        .map(|o| o.status.success())
                        .unwrap_or(false)
                } else {
                    // Check natively
                    self.check_browser_native(browser).await
                }
            } else {
                // Check natively
                self.check_browser_native(browser).await
    ...
    /// Check if a browser is available natively (not in PRoot).
    async fn check_browser_native(&self, browser: &str) -> bool {
    ...
    async fn resolve_browser(&self, requested: Option<&str>) -> anyhow::Result<String> {
        ...
        if let Some(browser) = requested {
            ...
            let installed = if let Some(ref proot) = self.proot_config {
                if proot.enabled {
                    proot.execute_command("which", &[&browser])
                        .await
                        .map(|o| o.status.success())
        ...
        if let Some(ref preferred) = self.preferred_browser {
            ...
            if SUPPORTED_BROWSERS.contains(&preferred.as_str()) {
                let installed = if let Some(ref proot) = self.proot_config {
                    if proot.enabled {
                        proot.execute_command("which", &[&preferred])
                            .await
                            .map(|o| o.status.success())
                            .unwrap_or(false)
                    } else {
                        self.check_browser_native(&preferred).await
                    }
                } else {
                    self.check_browser_native(&preferred).await
                };
...
impl Tool for TextBrowserTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // Execute command (with or without PRoot)
        let result = if let Some(ref proot) = self.proot_config {
            if proot.enabled {
                // Execute inside PRoot
                let args_str: Vec<&str> = dump_args.iter().map(|s| s.as_str()).collect();
                let proot_result = tokio::time::timeout(
                    timeout,
                    proot.execute_command(&browser, &args_str)
                )
                .await;
                
                // Convert Result<Result<Output, anyhow::Error>, Elapsed> to Result<Result<Output, io::Error>, Elapsed>
                match proot_result {
