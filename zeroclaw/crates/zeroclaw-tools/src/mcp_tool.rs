...
impl Tool for McpToolWrapper {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        tracing::debug!(tool = %self.prefixed_name, "MCP tool executing");
        match self.registry.call_tool(&self.prefixed_name, args).await {
            Ok(output) => {
                let elapsed = start.elapsed();
                tracing::info!(tool = %self.prefixed_name, duration_ms = %elapsed.as_millis(), "MCP tool succeeded");
                Ok(ToolResult {
                    success: true,
                    output,
                    error: None,
            ...
            Err(e) => {
                let elapsed = start.elapsed();
                tracing::error!(tool = %self.prefixed_name, error = %e, duration_ms = %elapsed.as_millis(), "MCP tool failed");
                Ok(ToolResult {
                    success: false,
                    output: String::new(),
