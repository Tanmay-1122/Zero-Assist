    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        if results.is_empty() {
            return Ok(ToolResult {
                success: true,
                output: format!(
                    "No deferred tools matched query `{query}`. The MCP catalog is loaded, \
                     but this query may be too narrow or use different wording. Try broader \
                     app/action keywords, increase max_results, or use select:<exact_tool_name> \
        ...
        let mut output = String::from(
            "tool_search only activates tool schemas; it has not executed any external action.\n<functions>\n",
        );
...
impl ToolSearchTool {
    fn select_tools(&self, names: &[&str]) -> anyhow::Result<ToolResult> {
        ...
        for name in names {
            ...
            match self.deferred.resolve_name(name) {
                Some(resolved_name) => {
                    let Some(spec) = self.deferred.tool_spec(resolved_name) else {
                        not_found.push(*name);
                        continue;
                    };
                    let resolved_name = resolved_name.to_string();
                    if !guard.is_activated(&resolved_name)
                        && let Some(tool) = self.deferred.activate(&resolved_name)
                    {
                        guard.activate(resolved_name, Arc::from(tool));
                        activated_count += 1;
                    }
                    let _ = writeln!(
                        output,
...
mod tests {
    ...
    #[tokio::test]
    async fn select_resolves_unique_unprefixed_mcp_suffix() {
        let activated = Arc::new(Mutex::new(ActivatedToolSet::new()));
        let tool = ToolSearchTool::new(
            make_deferred_set(vec![make_stub(
        ...
        let result = tool
            .execute(serde_json::json!({"query": "select:GMAIL-SEND-EMAIL"}))
            .await
            .unwrap();

        assert!(result.success);
        assert!(result.output.contains("composio__GMAIL_SEND_EMAIL"));
        assert!(!result.output.contains("Not found"));
        assert!(
            activated
                .lock()
                .unwrap()
                .is_activated("composio__GMAIL_SEND_EMAIL")
        );
    }
    ...
    async fn keyword_search_no_matches() {
        ...
        let result = tool
            ...
            .unwrap();
        assert!(result.success);
        assert!(result.output.contains("No deferred tools matched"));
        assert!(
            !result.output.to_ascii_lowercase().contains("not found"),
            "keyword no-match should not look like a missing exact tool"
        );
    }
