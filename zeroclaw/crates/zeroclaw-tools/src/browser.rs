...
use crate::proot_executor::ProotConfig;
use anyhow::Context;
...
pub struct BrowserTool {
    ...
    proot_config: Option<ProotConfig>,
...
impl BrowserTool {
    ...
    /// Set PRoot configuration for executing browser commands inside PRoot.
    pub fn with_proot_config(mut self, config: ProotConfig) -> Self {
        self.proot_config = Some(config);
        self
    }
    ...
    async fn run_command(&self, args: &[&str]) -> anyhow::Result<AgentBrowserResponse> {
        ...
        // Check if we should use PRoot
        if let Some(ref proot) = self.proot_config {
            if proot.enabled {
                return self.run_command_proot(agent_browser_bin, args, proot).await;
            }
        }
        
        // Native execution (no PRoot)
        ...
        self.parse_agent_browser_output(output)
    }
    
    /// Execute agent-browser command inside PRoot
    async fn run_command_proot(
        &self,
        ...
        let mut proot_args = Vec::new();
        
        // Add session if configured
        if let Some(ref session) = self.session_name {
            proot_args.push("--session");
            proot_args.push(session.as_str());
        }
        
        // Add action arguments
        proot_args.extend(args.iter().map(|s| *s));
        
        // Add --json for machine-readable output
        proot_args.push("--json");
        
        debug!("Running in PRoot: agent-browser {} --json", args.join(" "));
        
        let output = proot.execute_command(agent_browser_bin, &proot_args).await?;
        self.parse_agent_browser_output(output)
    }
    
    /// Parse agent-browser output into structured response
    fn parse_agent_browser_output(&self, output: std::process::Output) -> anyhow::Result<AgentBrowserResponse> {
    ...
    /// Execute a browser action with the given backend.
    /// This is the main entry point called from the Tool trait implementation.
...
impl Tool for BrowserTool {
    ...
    async fn execute(&self, args: Value) -> anyhow::Result<ToolResult> {
        ...
        // Delegate to the main execution method in impl BrowserTool
        self.execute_browser_action(args, backend).await
    }
}
