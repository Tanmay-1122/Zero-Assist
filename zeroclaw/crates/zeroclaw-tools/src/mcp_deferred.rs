...
const SEARCH_STOPWORDS: &[&str] = &[
    "a", "an", "and", "any", "are", "can", "do", "does", "for", "from", "i", "in", "is", "it",
    "me", "of", "on", "please", "that", "the", "then", "there", "this", "to", "use", "we", "what",
    "with", "you",
];
...
impl DeferredMcpToolSet {
    ...
    /// Resolve an exact prefixed MCP name, or a unique normalized unprefixed suffix.
    pub fn resolve_name(&self, name: &str) -> Option<&str> {
        if let Some(stub) = self.get_by_name(name) {
            return Some(stub.prefixed_name.as_str());
        }
        if name.contains("__") {
            return None;
        }

        let target = normalize_tool_name_for_lookup(name);
        if target.is_empty() {
            return None;
        }
        ...
        for stub in &self.stubs {
            ...
            if normalize_tool_name_for_lookup(suffix) != target {
                continue;
            }
            if resolved.is_some() {
                return None;
            }
            resolved = Some(stub.prefixed_name.as_str());
        }
        resolved
    }
    ...
    /// weighted term matches, then by name for deterministic broad searches.
    pub fn search(&self, query: &str, max_results: usize) -> Vec<&DeferredMcpToolStub> {
        let terms = normalize_search_terms(query);
        if terms.is_empty() {
            let mut stubs: Vec<&DeferredMcpToolStub> = self.stubs.iter().collect();
            stubs.sort_by(|a, b| a.prefixed_name.cmp(&b.prefixed_name));
            return stubs.into_iter().take(max_results).collect();
        }
        ...
        let mut scored: Vec<(&DeferredMcpToolStub, usize)> = self
            ...
            .filter_map(|stub| {
                ...
                let haystack_tokens = tokenize_search_text(&haystack);
                let score = terms
                    .iter()
                    .map(|term| {
                        if haystack.contains(term.as_str()) {
                            3
                        } else if haystack_tokens.iter().any(|token| token == term) {
                            2
                        } else if haystack_tokens.iter().any(|token| {
                            token.starts_with(term.as_str()) || term.starts_with(token.as_str())
                        }) {
                            1
                        } else {
                            0
                    ...
                    .sum();
                if score > 0 { Some((stub, score)) } else { None }
            })
            .collect();

        scored.sort_by(|(left_stub, left_score), (right_stub, right_score)| {
            right_score
                .cmp(left_score)
                .then_with(|| left_stub.prefixed_name.cmp(&right_stub.prefixed_name))
        });
        scored
            .into_iter()
            .take(max_results)
...
fn normalize_search_terms(query: &str) -> Vec<String> {
    ...
    for token in tokenize_search_text(query) {
        if SEARCH_STOPWORDS.contains(&token.as_str()) {
            continue;
        }
        push_unique_term(&mut terms, token.clone());
        if let Some(singular) = singularize_search_term(&token) {
            push_unique_term(&mut terms, singular);
        }
        for synonym in search_synonyms(&token) {
            push_unique_term(&mut terms, synonym.to_string());
        }
    }
    terms
}
...
fn singularize_search_term(term: &str) -> Option<String> {
    if term.len() > 4 && term.ends_with("ies") {
        return Some(format!("{}y", &term[..term.len() - 3]));
    }
    if term.len() > 3 && term.ends_with('s') {
        return Some(term[..term.len() - 1].to_string());
    }
    None
}

fn search_synonyms(term: &str) -> &'static [&'static str] {
    match term {
        "caption" | "captions" => &["subtitle", "subtitles", "transcript", "transcripts"],
        "statistic" | "statistics" => &["stat", "stats", "analytics", "metric", "metrics"],
        "stat" | "stats" => &["statistic", "statistics", "analytics", "metric", "metrics"],
        "mail" | "email" => &["gmail", "message"],
        "send" => &["create", "insert"],
...
mod tests {
    ...
    #[test]
    fn keyword_search_ignores_stopwords_for_broad_app_queries() {
        let stubs = vec![
            make_stub("composio__GMAIL_SEND_EMAIL", "Send email through Gmail"),
            make_stub(
                "composio__YOUTUBE_CAPTIONS_LIST",
                "List captions for a YouTube video",
            ),
            make_stub(
                "composio__YOUTUBE_COMMENT_INSERT",
        ...
        let results = set.search("what can we do with youtube", 10);
        assert_eq!(results.len(), 2);
        assert!(
            results
                .iter()
                .all(|stub| stub.prefixed_name.contains("YOUTUBE"))
        );
    }

    #[test]
    fn keyword_search_matches_youtube_synonyms() {
        let stubs = vec![
            make_stub(
                "composio__YOUTUBE_GET_CHANNEL_STATS",
                "Get YouTube channel metrics",
            ),
            make_stub(
                "composio__YOUTUBE_TRANSCRIPT_LIST",
                "List YouTube subtitles",
            ),
            make_stub("composio__YOUTUBE_COMMENT_INSERT", "Create a YouTube reply"),
        ];
        let set = DeferredMcpToolSet {
            stubs,
            registry: std::sync::Arc::new(
        ...
        let results = set.search("get channel statistics", 10);
        assert!(!results.is_empty());
        assert_eq!(
            results[0].prefixed_name,
            "composio__YOUTUBE_GET_CHANNEL_STATS"
        );

        let results = set.search("list captions on the video", 10);
        assert!(!results.is_empty());
        assert_eq!(
            results[0].prefixed_name,
            "composio__YOUTUBE_TRANSCRIPT_LIST"
