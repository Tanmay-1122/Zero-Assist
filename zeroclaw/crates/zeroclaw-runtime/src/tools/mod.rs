...
pub use zeroclaw_tools::mcp_client::{McpRegistry, McpServerStatus, sanitize_identifier};
...
use crate::capabilities::resolver::{CapabilityResolver, CapabilityTool};
...
pub fn default_tools_with_runtime(
    ...
    let (_cap_registry, cap_resolver, _logger, _shell_runtime) =
        crate::capabilities::build_default_registry(
            security.clone(),
            runtime,
            60,
        );
    ...
    // Create CapabilityTools for all enabled capabilities (driven by the registry)
    for summary in cap_resolver.list_capabilities() {
        let cap_tool = CapabilityTool::new(
            &summary.stable_name,
            Arc::new(cap_resolver.clone()),
        );
        tools.push(Box::new(RateLimitedTool::new(
            PathGuardedTool::new(cap_tool, security.clone()),
            security.clone(),
        )));
    }

    // Add non-capability tools
...
pub fn all_tools(
    ...
    CapabilityResolver,
    Arc<std::sync::Mutex<crate::capabilities::shell_runtime::ShellRuntime>>,
) {
...
pub fn all_tools_with_runtime(
    ...
    // Build capability registry + resolver (CapabilityTools created below)
    let (cap_registry, cap_resolver, _cap_logger, shell_runtime) =
        crate::capabilities::build_default_registry(
            security.clone(),
            runtime.clone(),
    ...
    // Create CapabilityTools for all enabled capabilities via the resolver
    for summary in cap_resolver.list_capabilities() {
        let cap_tool = CapabilityTool::new(
            &summary.stable_name,
            Arc::new(cap_resolver.clone()),
        );
        let wrapped = Arc::new(RateLimitedTool::new(
            PathGuardedTool::new(cap_tool, security.clone()),
            security.clone(),
        ));
        // Shell goes first, others follow in priority order
        tool_arcs.push(wrapped);
    }

    // Add non-capability tools
    tool_arcs.push(Arc::new(FileReadTool::new(security.clone())) as Arc<dyn Tool>);
    ...
    // Device control tool — registered when the Android FFI layer has set a handler.
    if device_control::device_control_available() {
        tool_arcs.push(Arc::new(DeviceControlTool));
    }
...
mod tests {
    ...
    fn all_tools_excludes_browser_when_disabled() {
        ...
        let (tools, _, _, _, _, _, _, _, _) = all_tools(
