...
pub struct PromptContext<'a> {
    ...
    /// Runtime capability context (cwd, last exit code, job count, etc.)
    /// injected into the system prompt so the LLM is aware of current state.
    ...
    pub fn with_defaults() -> Self {
        Self {
            sections: vec![
                ...
                Box::new(CapabilityContextSection),
            ],
        }
    }
...
pub struct CapabilityContextSection;
...
impl PromptSection for CapabilityContextSection {
    fn name(&self) -> &str {
        "capability_context"
    }

    fn build(&self, ctx: &PromptContext<'_>) -> Result<String> {
        match ctx.capability_context {
            Some(ref context) if !context.is_empty() => {
                Ok(format!("## Shell Environment\n\n{context}\n"))
            }
            _ => Ok(String::new()),
        }
    }
