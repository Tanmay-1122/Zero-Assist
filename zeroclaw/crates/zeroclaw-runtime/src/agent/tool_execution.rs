...
/// Supports aliasing: `shell` automatically resolves to `sandbox_execute`.
pub fn find_tool<'a>(tools: &'a [Box<dyn Tool>], name: &str) -> Option<&'a dyn Tool> {
    // Try direct lookup first
    if let Some(tool) = tools.iter().find(|t| t.name() == name).map(|t| t.as_ref()) {
        return Some(tool);
    }
    
    // Apply tool name aliases for convenience
    let aliased_name = match name {
        "shell" => "sandbox_execute",
        _ => return None,
    };
    
    tools.iter().find(|t| t.name() == aliased_name).map(|t| t.as_ref())
}
...
pub struct ToolExecutionOutcome {
    ...
    /// Optional structured metadata (e.g. canvas frames) from the tool.
