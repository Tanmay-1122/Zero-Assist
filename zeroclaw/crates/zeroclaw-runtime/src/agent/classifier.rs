...
/// New trivial query fast-path

/// Detect trivial queries that don't need LLM reasoning.
/// Returns true for greetings, short acknowledgments, simple yes/no, etc.
pub fn is_trivial_query(message: &str) -> bool {
    ...
    if greetings.iter().any(|g| lower == *g || lower.starts_with(&format!("{},", g)) || lower.starts_with(&format!("{} ", g))) {
        return true;
    }
    ...
    let acknowledgments = [
        ...
        "ty", "thx", "k", "kk", "cool", "nice", "great", "awesome",
        "got it", "i see", "understood", "sure", "yeah", "yes", "no",
        "done", "perfect", "right", "correct",
    ];
    if acknowledgments.iter().any(|a| lower == *a || lower.trim() == *a) {
        return true;
    }
...
/// Enhanced classification that returns None for trivial queries.
pub fn classify_smart(
    config: &QueryClassificationConfig,
    message: &str,
) -> Option<String> {
    // Fast path: trivial queries don't need routing
