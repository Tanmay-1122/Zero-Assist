...
pub struct Agent {
    ...
    /// Shared shell runtime for capability context injection into system prompt.
    shell_runtime: Option<Arc<std::sync::Mutex<crate::capabilities::shell_runtime::ShellRuntime>>>,
}
...
impl AgentBuilder {
    ...
    pub fn shell_runtime(
        mut self,
        runtime: Option<Arc<std::sync::Mutex<crate::capabilities::shell_runtime::ShellRuntime>>>,
    ) -> Self {
        self.shell_runtime = runtime;
        self
    }
...
impl Agent {
    ...
    pub async fn from_config(config: &Config) -> Result<Self> {
        ...
        Agent::builder()
            ...
            .shell_runtime(Some(shell_runtime))
            .build()
    }
    ...
    fn build_system_prompt(&self) -> Result<String> {
        ...
        let capability_context = self.shell_runtime.as_ref().and_then(|rt| {
            rt.lock().ok().map(|r| r.context_string())
        });
            ...
            if let Some(tool) = self.tools.iter().find(|t| t.name() == tool_name) {
                match tool.execute(tool_args.clone()).await {
                    Ok(r) => {
                        ...
                        if r.success {
                            (r.output, true, r.metadata)
                        } else {
                            (format!("Error: {}", r.error.unwrap_or(r.output)), false, None)
                        }
                    }
                    Err(e) => {
                        self.observer.record_event(&ObserverEvent::ToolCall {
