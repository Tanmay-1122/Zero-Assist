...
/// A tool call validation error with a message suitable for LLM feedback.
#[derive(Debug, Clone)]
...
/// Validate parsed tool calls against available tool schemas.
///
/// Checks:
/// - Tool name exists in the registered tools
/// - Arguments are valid JSON objects (not arrays, strings, or null)
/// - No unexpected top-level properties (basic hallucination detection)
///
...
pub fn validate_tool_calls(
    ...
    for call in calls {
        ...
        let tool_spec = tool_specs.iter().find(|s| s.name == call.name);
        let Some(spec) = tool_spec else {
            errors.push(ToolCallValidationError {
                ...
                message: format!(
                    ...
                    call.name,
                    tool_specs.iter().map(|s| s.name.as_str()).collect::<Vec<_>>().join(", ")
                ),
            });
            continue;
        };

        // Check 2: Arguments must be a JSON object
        if !call.arguments.is_object() {
            errors.push(ToolCallValidationError {
                ...
                message: format!(
                    ...
                    call.name,
                    json_type_name(&call.arguments)
                ),
            });
            continue;
        }

        // Check 3: If the tool's parameter schema defines properties,
        // check for unexpected top-level keys (hallucination detection).
        if let Some(props) = spec.parameters.get("properties").and_then(|p| p.as_object()) {
            let args = call.arguments.as_object().unwrap();
            let unknown_keys: Vec<&String> = args.keys().filter(|k| !props.contains_key(*k)).collect();
            if !unknown_keys.is_empty() {
                let expected_keys: Vec<&String> = props.keys().collect();
                errors.push(ToolCallValidationError {
                    tool_name: call.name.clone(),
                    message: format!(
                        "Unexpected parameter(s) for '{}': {}. Expected parameters: {}",
                        call.name,
                        unknown_keys.iter().map(|k| k.as_str()).collect::<Vec<_>>().join(", "),
                        expected_keys.iter().map(|k| k.as_str()).collect::<Vec<_>>().join(", "),
                    ),
                });
            }

            // Check 4: Required parameters are present
...
/// A simplified ToolSpec for validation (mirrors zeroclaw_api::tool::ToolSpec
/// without creating a dependency on the API crate).
#[derive(Debug, Clone)]
pub struct ToolSpec {
    pub name: String,
