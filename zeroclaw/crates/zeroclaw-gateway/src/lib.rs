...
pub struct AppState {
    ...
    /// Whether the web dashboard is enabled (toggleable at runtime)
    pub web_enabled: Arc<std::sync::atomic::AtomicBool>,
...
pub async fn run_gateway(
    ...
    if !config.mcp.servers.is_empty() && !config.mcp.enabled {
        tracing::warn!(
            "Gateway: {} MCP server(s) configured but MCP is not enabled — set `mcp.enabled = true`",
    ...
    if config.mcp.enabled && !active_mcp_servers.is_empty() {
        tracing::info!(
            "Gateway: initializing MCP client — {} of {} server(s) active",
    ...
    let state = AppState {
        ...
        web_dist_dir,
        web_enabled: Arc::new(std::sync::atomic::AtomicBool::new(true)),
        canvas_store,
    ...
    let inner = Router::new()
        ...
        .route("/admin/web/toggle", post(handle_admin_web_toggle))
        .route("/admin/web/status", get(handle_admin_web_status))
        // ── Existing routes ──
...
/// POST /admin/web/toggle — toggle web dashboard on/off
async fn handle_admin_web_toggle(
    State(state): State<AppState>,
) -> impl IntoResponse {
    use std::sync::atomic::Ordering;
    let current = state.web_enabled.load(Ordering::Relaxed);
    state.web_enabled.store(!current, Ordering::Relaxed);
    tracing::info!(
        "🌐 Web dashboard {} via admin toggle",
        if current { "disabled" } else { "enabled" }
    );
    Json(serde_json::json!({
        "web_enabled": !current,
        "message": if current { "Web dashboard disabled" } else { "Web dashboard enabled" }
    }))
}

/// GET /admin/web/status — check if web dashboard is enabled
async fn handle_admin_web_status(
    State(state): State<AppState>,
) -> impl IntoResponse {
    use std::sync::atomic::Ordering;
    Json(serde_json::json!({
        "web_enabled": state.web_enabled.load(Ordering::Relaxed)
    }))
}
